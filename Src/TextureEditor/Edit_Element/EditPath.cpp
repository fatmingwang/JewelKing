...
namespace FMCEditor 
{
	System::Void EditPath::AddPath_button_Click(System::Object^  sender, System::EventArgs^  e)
	{
		 ...
		 else
		 {
			...
			l_pForm->ShowDialog();
			const char*	l_String = l_pForm->m_strTextName->c_str();
			if( strlen(l_pForm->m_strTextName->c_str()) )
			{
				if(!DNCT::CheckListContainStringAndAdd(this->WholePath_listBox,String(l_pForm->m_strTextName->c_str()).ToString()))
				{
					...
					l_pcCurveWithTime->SetName(UT::CharToWchar(l_pForm->m_strTextName->c_str()));
	...
	System::Void EditPath::RefershPathData()
	{
		...
		{
			WholePath_listBox->Items->Add(String(UT::WcharToChar((*m_pCurveManagerEP)[i]->GetName())).ToString());
		}
	}
						 ...
						 if( sender == mergeToolStripMenuItem )
						 {
							...
							if( l_bReplace )
							{
								...
								{//if name is same erase data and add it
									m_pCurveManagerEP->ReplaceObject(l_CurveManager.GetObject(i));
		...
		if( !Play_checkBox->Checked )
		{
			if( ShowImage_checkBox->Checked )
			{
				if( ImageName_listBox->SelectedIndex != -1 )
				{
					...
					l_pImage->Render(l_Pos.x-(l_pImage->GetImageWidth()/2),l_Pos.y-(l_pImage->GetImageHeight()/2));
				}
			}
		}
		else
