...
//
//=======================
	cUIImage*	cPIEditor::GetNewUIImageByBitMap(System::Drawing::Bitmap^e_pBitMap,WCHAR*e_strName)
	{
	...
	{
		cObjectListByName<cPuzzleUnitChild>	l_PIUnitChildVector;
		...
		try
		 ...
		 ImageWidth_numericUpDown->Value = e_pPuzzleImage->GetImageWidth();
		 ImageHeight_numericUpDown->Value = e_pPuzzleImage->GetImageHeight();
		 SAFE_DELETE(e_pPuzzleImage);
	...
	void			cPIEditor::OpenPIFile(String^e_strFileName)
	{
		 ...
		 if( l_strFileName )
		 {
				...
				if( l_pPuzzleImage )
				{
				    cObjectListByName<cImageIndexOfAnimation>*l_pImageIndexOfAnimationList =l_pPuzzleImage->GetImageIndexOfAnimationList();
...
//
//=======================
	System::Void	cPIEditor::SavePuzzleFile()
	{
		...
		for( int i=0;i<m_pImageomposerIRM->Count();++i )
		{
		 ...
		 if( l_FileName )
		 {
			...
			char*l_psFileExtension = ".png";
			int	l_iWidth = (int)ImageWidth_numericUpDown->Value;
			int	l_iHeight = (int)ImageHeight_numericUpDown->Value;

			System::Drawing::Bitmap^l_pBitMap = gcnew System::Drawing::Bitmap( l_iWidth,l_iHeight,System::Drawing::Imaging::PixelFormat::Format32bppArgb );
			...
			l_XMLWriter.StartElement("PuzzleImage");
			    ...
			    for( int i=0;i<AllImage_listBox->Items->Count;++i )
			    {
				    ...
				    //int	l_iPixelSize = l_pUIImage->GetImageWidth()*l_pUIImage->GetImageHeight()*4;
				    ...
				    l_XMLWriter.StartElement("PuzzleUnit");
						...
						assert(l_OriginaleSize.x>=l_ImageRealPixelSize.x);
						assert(l_OriginaleSize.y>=l_ImageRealPixelSize.y);
			...
			if( !l_sbCollide )
			{
				for( int i=m_pImageomposerIRM->Count()-1;i!=-1;--i )
				{
					...
					RECT	l_rc = {
						...
						(int)(l_rc.left+l_pUIImage->GetImageWidth()),
						(int)(l_rc.top+l_pUIImage->GetImageHeight())};
		...
		else
		{
			if( GCFORM::MouseButtons::None == e->Button )
			{
				if( l_siCollideIndex!=-1 )
				{
					...
					//m_pParticlePropertyGrid->Size = System::Drawing::Size(l_pUIImage->GetImageWidth(),l_pUIImage->GetImageHeight());
