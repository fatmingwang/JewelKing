...
//	m_pMPDI_Slot = 0;
	...
	if( e_nStage==MINI_GAME_STAGE_01 )
	{
		m_nCountMessage = 0;
	}
	else if( e_nStage==MINI_GAME_STAGE_02 )
	{
		m_nCountMessage = 0;
	}
	else if( e_nStage==MINI_GAME_STAGE_03 )
	{
		m_nCountMessage = 0;
	}
	else if( e_nStage==MINI_GAME_STAGE_04 )
	{
		m_nCountMessage = 0;
	}
	else if( e_nStage==MINI_GAME_STAGE_31 )
	{
		m_nCountMessage = 0;
	}
	else if( e_nStage==MINI_GAME_STAGE_32 )
	{
		m_nCountMessage = 3;
	}
	else if( e_nStage==MINI_GAME_STAGE_33 )
	{
		m_nCountMessage = 0;
	} //end if
	m_pMPDI_WeaponGetMessage = 0;
	if( m_nCountMessage>0 )
	{
		m_pMPDI_WeaponGetMessage	= new cMPDI*[m_nCountMessage];
		for( int ii=0; ii<m_nCountMessage; ii++ )
		{
...
void	cMiniGameBase::Init(void)
{
	...
	else if( m_nStage==MINI_GAME_STAGE_31 )
	{
		m_nCount_Fire01 = 1;
		m_nCount_Fire01_Current = m_nCount_Fire01;
	}
	else if( m_nStage==MINI_GAME_STAGE_32 )
	{
		m_nCount_Fire01 = 1;
		m_nCount_Fire01_Current = m_nCount_Fire01;
	}
	else if( m_nStage==MINI_GAME_STAGE_33 )
	{
		m_nCount_Fire01 = 1;
		m_nCount_Fire01_Current = m_nCount_Fire01;
	} //end if
...
//		m_pMPDI_Slot->Init();
	...
	{
		l_pMPDIList_Bonus = cGameApp::GetMPDIListByFileName( (WCHAR*)MINI_GAME_BONUS_RUNNER );
		//
		m_pMPDI_WeaponGetMessage[0] = new cMultiPathDynamicImage( l_pMPDIList_Bonus->GetObject(L"LuckRunnerShow") );
		m_pMPDI_WeaponGetMessage[0]->Init();
		m_pMPDI_WeaponGetMessage[1] = new cMultiPathDynamicImage( l_pMPDIList_Bonus->GetObject(L"LuckRunnerStart") );
		m_pMPDI_WeaponGetMessage[1]->Init();
		m_pMPDI_WeaponGetMessage[2] = new cMultiPathDynamicImage( l_pMPDIList_Bonus->GetObject(L"LuckRunner") );
		m_pMPDI_WeaponGetMessage[2]->Init();
		//
		m_pMPDI_Plate = new cMPDI( l_pMPDIList_Bonus->GetObject( L"BonusShow") );
		m_pMPDI_Plate->Init();
		m_pMPDI_PlateExplode = new cMPDI( l_pMPDIList_Bonus->GetObject(L"BonusShowBreak") );
...
void	cMiniGameBase::Update( float e_fElpaseTime )
{
	...
	if( m_nFlag==MINI_GAME_FLAG_STICKY_MONSTER ) //圓盤黏著魚
	{
		...
		{
			m_pStickyMonster->SetMonsterStatus( eMS_WAITING_FOR_CLEAN );
		} //end if
		//
		//有加分的魚被打死
...
//		m_pMPDI_BonusShow_D->Init();
	...
	//
	m_nFlag = e_nFlag;
	Update(0.0f); //設定完 Flag 之後，馬上要在 Update 一次，Render 才會準。
	//
	return;
}
