...
{
	TC.SetTargetTime(3.f);
	iTotalMoney = 0;
	iBulletCount = 1;
	iBulletFireMoneyAeverage = 5;
}

void	sBulletFireAverage::BulletFire(int e_iPayRate)
{
	iTotalMoney += e_iPayRate;
	++iBulletCount;
}
...
void	sBulletFireAverage::Update(float e_fElpaseTime)
{
	...
	{
		iBulletFireMoneyAeverage = iTotalMoney/(iBulletCount);
		iTotalMoney = 0;
		iBulletCount = 1;
		TC.Start();
	}
...
float	sBulletFireAverage::RelativeProbability(int e_iPayRate,int e_iOneBulletHitMonsterMonut)
{
	...
	float	l_fValue = 1.f/(l_iOffset+(e_iOneBulletHitMonsterMonut*2));
	return l_fValue;
}
...
void	cProbabilityData::ProcessRevenueProbabilityData(TiXmlElement*e_pTiXmlElement)
{
	PARSE_ELEMENT_START(e_pTiXmlElement)
		...
		{
			m_iHappyRevenue = VALUE_TO_INT*(*this->m_piCoinToScore);
		}
		else
