...
void	cFishGroup::FishGroupGoLeftToRight(int e_iIndex,float e_fTime)
{
	...
	FishGroupGo(e_iIndex,l_TargetPath);
}
...
RETURNS	      : 
\* ============================================================================================= */
void	cFishGroup::FishGroupGo(int e_iIndex,cCurveWithTime e_TargetPath)
{
	...
	if( l_pFishGroupPointData )
	{
		...
		for( size_t	i=0;i<l_uiSize;++i )
		{
			...
			{
				cMonster*l_pMonster = cFishApp::m_spMonsterManager->Require(l_iFishId);
				if( !l_pMonster )
					break;
...
RETURNS	      : 
\* ============================================================================================= */
void	cFishGroup::FishGroupGo(int e_iIndex)
{
	cCurveWithTime	l_TargetPath;
	cFishGroup::cFishGroupPointData*l_pFishGroupPointData = this->m_FishGroup.GetObject(e_iIndex);
