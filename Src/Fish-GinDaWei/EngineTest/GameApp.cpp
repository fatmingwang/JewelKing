...
#endif
{
	...
	if(!g_pIOSMDriverInterface)
	{
		...
		if(g_pIOSMDriverInterface->Parse("Fish-GinDaWei/IO/IOData.xml"))
			this->m_spLogFile->WriteToFileImmediatelyWithLine(L"Fish-GinDaWei/IO/IOData-file.xml ok");
		else
			this->m_spLogFile->WriteToFileImmediatelyWithLine(L"Fish-GinDaWei/IO/IOData-file.xml falied");
	}
	if(!cGameApp::m_spSoundParser->Parse("Fish-GinDaWei/Sound/Sound.xml"))
	{
		UT::ErrorMsg(L"Fish-GinDaWei/Sound.xml",L"parse sound error");
	}
...
void	cFishApp::Init()
{
	...
	if( !m_spControlPanel )
	{
		m_spControlPanel = new cControlPanel("Fish-GinDaWei/ControlPanel/EventPachages.xml");
...
void	TimsStamp(eDebugInfo e_eDebugInfo,bool e_bUpdate)
{
	g_TimeStamp.Update();
	if( e_bUpdate )
	{
...
void	cFishApp::Render()
{
	...
	else
	{
		...
		UT::sTimeAndFPS	l_FPS2;
		l_FPS2.Update();
		l_FPS.Update();
		cGameApp::Render();
		l_FPS.Update();
		g_fRenderTime[eDI_GAMEAPP] = l_FPS.fElpaseTime;

		if( m_spSceneChange )
			m_spSceneChange->Render();
		l_FPS.Update();		
		g_fRenderTime[eDI_SCENE_CHANGE] = l_FPS.fElpaseTime;

		if( m_spMiniGameManager )
			m_spMiniGameManager->Render();
		l_FPS.Update();
		g_fRenderTime[eDI_MINI_GAME] = l_FPS.fElpaseTime;

		if( m_spMonsterManager )
			m_spMonsterManager->Render();
		l_FPS.Update();
		g_fRenderTime[eDI_MONSTER] = l_FPS.fElpaseTime;
		...
		l_FPS.Update();
		g_fRenderTime[eDI_BULLET] = l_FPS.fElpaseTime;

		if( m_spPlayerManager )
			m_spPlayerManager->Render();
		l_FPS.Update();
		g_fRenderTime[eDI_PLAYER] = l_FPS.fElpaseTime;
		...
		l_FPS.Update();
		g_fRenderTime[eDI_PROBABILITY] = l_FPS.fElpaseTime;

		if(m_pRefoundCoinDeviceManager)
			m_pRefoundCoinDeviceManager->Render();
		l_FPS.Update();
		g_fRenderTime[eDI_REFOUND_DEVICE] = l_FPS.fElpaseTime;
		...
		g_fRenderTime[eDI_SCENE_CHANGE] += l_FPS.fElpaseTime;
		l_FPS2.Update();
		float	l_fRenderTime = l_FPS2.fElpaseTime;
		glEnable2D(cGameApp::m_svGameResolution.x,cGameApp::m_svGameResolution.y);
		...
		if( g_sbCollisionRender )
		{
			...
			l_FPS.Update();
			g_fRenderTime[eDI_IO] = l_FPS.fElpaseTime;
			...
			l_FPS2.Update();
			l_fRenderTime = l_FPS2.fElpaseTime;
