...
namespace IOSM
{
	const wchar_t*         cIOSMIOData::TypeID( L"cIOSMIOData" );
	const wchar_t*         cIOSMIOPackData::TypeID( L"cIOSMIOPackData" );
	...
	void	cIOSMIOData::Init()
	{
		...
		else
		{
			...
			else
			{
				...
				if(!g_pIOSMDriverInterface->ReadInputData(this->m_dwMemoryAddress,&m_bCurrentSingnale))
				{
...
//	swprintf(l_strInfo,MAX_PATH,L"Name:%ls,Singnal:%d,BitAddress:%d,MemoryAdress:%d,Bounce:%f\0",GetName(),m_bCurrentSingnale?1:0,(int)m_byBitAddress,(int)m_dwMemoryAddress,this->m_fBounce);
	...
	cIOSMIOPackData::cIOSMIOPackData()
	{
		m_iDataSize = 0;
		m_pData = 0;
		m_bRead = true;
	...
	void	cIOSMIOPackData::SetDataSize(int	e_iSize)
	{
		if( m_pData )
			delete m_pData;
		m_pData = malloc(e_iSize);
		memset(m_pData,0,e_iSize);
		this->m_iDataSize = e_iSize;
	}

	void	cIOSMIOPackData::Update(float e_fElpaseTime)
	{
		if( m_bRead )
		{	
			...
			{
				char	l_Value = 0;
				if(g_pIOSMDriverInterface->ReadIOChar(this->m_dwMemoryAddress,&l_Value));
				{
					//if( l_Value != 0 )
					//{
					//	OutputDebugString(L" \n");
					//	OutputDebugString(this->GetName());
					//	OutputDebugString(L",Address:");
					//	OutputDebugString(ValueToStringW((int)m_dwMemoryAddress));
					//	OutputDebugString(L" Error\n");
					//}
					memcpy(m_pData,&l_Value,sizeof(char));
				}
			}
			else
			if(this->m_iDataSize == 4)
			{
				if(g_pIOSMDriverInterface->ReadIO4ByteOnce(this->m_dwMemoryAddress,(int*)m_pData))
				{
			...
			{
				if(g_pIOSMDriverInterface->ReadIORange(this->m_dwMemoryAddress,m_iDataSize,(char*)m_pData))
				{
...
/*			if(this->m_iDataSize == 1)
			{
				if(g_pIOSMDriverInterface->WriteIOChar(this->m_dwMemoryAddress,(char)(char*)m_pData))
				{
			...
			if(this->m_iDataSize == 4)
			{
				if(g_pIOSMDriverInterface->WriteIO4ByteOnce(this->m_dwMemoryAddress,(int)(int*)m_pData))
				{
			...
			{
				if(g_pIOSMDriverInterface->WriteIORange(this->m_dwMemoryAddress,(char*)m_pData,m_iDataSize))
				{
	...
	int				cIOSMIOPackData::GetPackDataByInt()
	{
		return *(int*)m_pData;
	}

	unsigned char	cIOSMIOPackData::GetPackDataByByte()
	{
		m_bRead = true;
		return *(unsigned char*)m_pData;
	}

	bool	cIOSMIOPackData::SetPackDataByByte(unsigned char	e_iRestCoin)
	{
		m_bRead = false;
		*(unsigned char*)m_pData = e_iRestCoin;
		return g_pIOSMDriverInterface->WriteIOChar(m_dwMemoryAddress,e_iRestCoin);
	}
	//====================
	...
	void	cIOSMIODataManager::ProcessIOPackData()
	{
		cIOSMIOPackData*l_pIOSMIOData = new cIOSMIOPackData();
	...
	void	cIOSMIODataManager::ProcessIOData()
	{
		...
		PARSE_NAME_VALUE_START
			...
			{
				unsigned char*l_pucKeyData = new unsigned char;
				*l_pucKeyData = (unsigned char)VALUE_TO_INT;
				int	l_iValue = (int)(*l_pucKeyData);
				...
				OutputDebugString(l_pIOSMIOData->GetName());
				OutputDebugString(L" Value:");
				WCHAR	l_Test[2] = {l_iValue,L'\0'};
				OutputDebugString(ValueToStringW(l_iValue));
				OutputDebugString(L",");
				OutputDebugString(l_Test);
				OutputDebugString(L"\n");
#endif
#endif
				l_pIOSMIOData->SetKeyData(l_pucKeyData);
