...
{
//	m_iStoreHigherThanAeverageRevenue = 0;
	...
	cEventVariable*l_pBulletPayRateLimit =	cFishApp::m_spControlPanel->GetEventManager()->m_pEventVariableManager->GetObject(L"BulletPayRateLimit");
	...
	m_piBulletLimit = l_pBulletPayRateLimit->GetInt();
	...
	m_iHappyRevenue = Vector2((float)m_iFakeRevenue,(float)m_iFakeRevenue);
	...
	m_bCurrentRoundFakeRevenueGet = false;
}
...
void	cProbabilityData::RevenueMinus(int e_iPayRate,int e_iMoney,int e_iPlayerID,bool e_bMiniGame)
{
	*m_pi64LatestOutMoney			+=		e_iMoney;
	*m_pi64TotalOutMoney			+=		e_iMoney;
	if(this->m_bCurrentRoundFakeRevenueGet)
	{
	...
	if( !e_bMiniGame )
		this->m_BulletData.RevenueMinus(e_iMoney,e_iPayRate);
}
...
void	cProbabilityData::RevenueAdd(int e_iMoney,int e_iPlayerID)
{
	...
	//if(e_iMoney>m_BulletFireAverage.iBulletFireMoneyAeverage)
	//{
	//	this->m_iStoreHigherThanAeverageRevenue += e_iMoney;
	//}
	...
	*m_pi64TotalInMoney					+=	e_iMoney;
	...
	float l_fAteMoney = m_MiniGameData.fIncomePercentage*e_iMoney;
	for( size_t i=0;i<m_MiniGameData.MiniGameBankList.size();++i )
	{
		l_fMiniGameStoreMoney += l_fAteMoney;
		m_MiniGameData.MiniGameBankList[i].fAteMoney += l_fAteMoney;
	...
	this->m_BulletData.RevenueAdd(e_iMoney - (int)l_fMiniGameStoreMoney);
}
...
int64		cProbabilityData::GetLeatestInMoney()
{
	...
	else
	{
		...
		{
			m_bCurrentRoundFakeRevenueGet = true;
			//l_FakeMoneyFetch = rand(0,m_iFakeRevenue/rand(1,m_iFakeRevenueGiveDivision));
			l_FakeMoneyFetch = rand(m_iFakeRevenue/m_iFakeRevenueGiveDivision,m_iFakeRevenue);
...
void	cProbabilityData::FakeRevenueUpdate(float e_fElpaseTime)
{
	...
	if( m_FakeRevenueTC.bTragetTimrReached )
	{
		//if( m_iFakeRevenue <= 0 )
		{
			...
			if(IsProbailitySafeToOut(this->GetCurrentProbability()))
			{
				m_iFakeRevenue = m_iHappyRevenue.Rand();
			}
			if(this->IsHappyTime())
...
{
	m_BulletData.Update(e_fElpaseTime);
...
void	cProbabilityData::Render()
{
	...
	cGameApp::m_spGlyphFontRender->RenderFont(0,410,l_str.c_str());

	l_str = UT::ComposeMsgByFormat(L"MiniGameMoney:%.2f",this->m_MiniGameData.fTotalAteMoney);
	cGameApp::m_spGlyphFontRender->RenderFont(620,450,l_str.c_str());
