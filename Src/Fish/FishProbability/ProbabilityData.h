...
struct	sMiniGameData
{
	...
	//fake mini gamemonster.
	UT::sMinMaxData<float>	fFakeMiniGameDuration;
	UT::sTimeCounter		FakeMiniGameMonsterTime;
...
class	cProbabilityData:public cNodeISAX
{
	...
	bool					m_bCurrentRoundFakeRevenueGet;//if fake is work minus fake money revenue,reset it every monster died check
	UT::sMinMaxData<float>	m_FakeRevenueTCRestrict;
	UT::sTimeCounter		m_FakeRevenueTC;
	int						m_iFakeRevenue;
	//while bullet fire is higher than aeverage store this.
	//int						m_iStoreHigherThanAeverageRevenue;
	UT::sMinMaxData<int>	m_iHappyRevenue;
	...
	cBulletDataAndBankRepositorManager				m_BulletData;
public:
	cProbabilityData();
	~cProbabilityData();
	void					Init();
	...
	int						GetHappyRevenue(){return m_iHappyRevenue.Rand();}
