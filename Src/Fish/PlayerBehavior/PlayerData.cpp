...
cPlayerData::cPlayerData( TiXmlElement*e_pElement )
{
	...
	for(int i=0;i<cCoinManager::COUNT_COINS;++i)
	{
		...
		m_StackShowMax = m_pCoinManager ->m_iStackShowMax;
	}
	...
	if( l_iStackPosIndex != -1 && m_iAddScoreCount <= 50 )
	{
		for(int i=0;i< cCoinManager::COUNT_COINS;++i)
		{
			if(m_pCoinManager->m_Coin[i]->m_eCointStatus==eCoinNull && Money > 0)
			{
				...
				m_pCoinManager->m_Coin[i]->m_vStacScorekPos = m_vStackScorePos[m_pCoinManager->StackIndex(Money)];
				if ( m_NowCoinStackNum < m_StackShowMax )
				{
					m_NowCoinStackNum++;
					m_NowAddPos++;
					...
					m_pCoinManager->m_Coin[i]->m_bRemoveStack = true;
...
void cPlayerData::ProcessPlayerIO( TiXmlElement*e_pElement )
{
	...
	m_FireClick.SetRepeatTime(0.3f);
...
void cPlayerData::Init(void)
{
	...
	else
	{
		...
		//把所有物件抓出來轉向
		int	l_iCount = m_pPlayerImage->Count();
		...
		m_pPlayerImage->GetObject(L"PlayerUI")->ChangeAllImageByImageIndex(l_iIndexPlayerUI);
		...
		Vector3 l_vScorePos = l_pAllPlayerPos->GetObject(L"PlayerScoreNumerial1")->GetOriginalPointList()[0];
		Vector3 l_vStackScorePos[4] ;
		l_vStackScorePos[0] = l_pAllPlayerPos->GetObject(L"CoinStack1")->GetOriginalPointList()[0];
		l_vStackScorePos[1] = l_pAllPlayerPos->GetObject(L"CoinStack2")->GetOriginalPointList()[0];
		l_vStackScorePos[2] = l_pAllPlayerPos->GetObject(L"CoinStack3")->GetOriginalPointList()[0];
		l_vStackScorePos[3] = l_pAllPlayerPos->GetObject(L"CoinStack4")->GetOriginalPointList()[0];

		//Vector3 l_vTubePos= m_pPlayerImage->GetObject(L"TubePos")->GetOriginalPointList()[0];
		Vector3 l_vTubePos= m_pPlayerImage->GetObject(L"TubePos")->GetOriginalPointList()[0] + Vector3( 98, 42.5, 0 );
		...
		for(int i = 0; i < 4; ++i )
		{
			m_vStackScorePos[i] = l_mat.TransformCoordinate(l_vStackScorePos[i]);
		}
	...
	m_pMessageInsert = new cMultiPathDynamicImage( l_pMessageMPDIList ->GetObject(L"PleaseInsertCoin_D") );
	m_pMessageNoMoney = new cMultiPathDynamicImage( l_pMessageMPDIList ->GetObject(L"MoneyNotEnough_D") );
	m_pMessageExplode = new cMultiPathDynamicImage( l_pMessageMPDIList ->GetObject(L"MoneyLimit_D") );
...
void cPlayerData::Update( float e_fElpaseTime )
{
	...
	//
	if( cGameApp::m_sucKeyData[this->m_cInput_Left] && !g_pIOSMDriverInterface->IsIOError())
	{
	...
	if( cGameApp::m_sucKeyData[this->m_cInput_Right] && !g_pIOSMDriverInterface->IsIOError() )
	{
		cGameApp::SoundPlay(L"2",true);
		if(m_fTubeTime<0.9f)
		{
	...
	if(m_LevelClick.IsSatisfiedCondition() && !g_pIOSMDriverInterface->IsIOError() )
	{
		m_LevelClick.Init();
...
//				eMiniGameFlag abc = m_pPlayerDataMiniGameCardList->GetObject(ii)->m_nFlag;
	...
	m_pPlayerImage->UpdateByGlobalTime(m_fTubeTime);
	m_pCoinManager->Update(e_fElpaseTime);
	UpdateFireStatusTransform();
}
...
void cPlayerData::Render(void)
{
		...
		(m_pCurrentMiniGame->f_GetFlag()<MINI_GAME_FLAG_GOTO_THUNDER_IDLE || m_pCurrentMiniGame->f_GetFlag()>MINI_GAME_FLAG_GOTO_THUNDER_FIRE_ED) )
	{
		if( this->m_eStatus == PLAYER_STATUS_IDLE )
		{
			m_pPlayerImage->RenderByGlobalTime();
		}
		else
		{	
			...
			m_AttackList.Render();
			m_pPlayerImage->GetObject( L"LvPos" )->Render();
...
void cPlayerData::UpdateFireStatusTransform()
{
	if( PLAYER_STATUS_ATTACK == m_eStatus )
		{
		Vector3 l_vAttackAngle = Vector3( 0, 0, 270 + (m_fTubeTime*180) );
		m_matTubeTransfomr = m_matPlayerTransfomr * cMatrix44::TranslationMatrix(Vector3(-24.5,-21.25,0)) * cMatrix44::RotationMatrix(l_vAttackAngle);
...
{
	//
	cMatrix44	l_mat = m_pPlayerImage->GetObject( L"TubePos" )->GetRenderMatrix(); //取得砲座位置
	...
	if( e_pBullet )
	{
		...
		Vector3 l_vOffsetDirection = Vector3::Zero;
		Vector2	l_vSize = e_pBullet->f_GetBulletMPDI()->GetDrawSize()/2.f;
		l_vOffsetDirection = l_mat.GetAxis(MyMath::Z)*l_vSize.x;
		l_vOffsetDirection.z = 0.f;
		if(m_iLevel == 0)
		{
			l_vDirection *= 90;
