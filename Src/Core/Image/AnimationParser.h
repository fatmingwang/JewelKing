...
namespace FATMING_CORE
{
	...
	//function inherit from cObjectListByName,like remove object replace and any else.
	...
	//cSAList or MPDIList
	class	cAnimationParser:public cObjectListByName<NamedTypedObject>,public ISAXCallback
	{
		...
		//internal using while parse data
		cSAList*m_pCurrentImagePlayer;
		//internal using
	...
	public:
		...
		cMPDIList*GetMPDIList(const WCHAR*e_strName){ return dynamic_cast<cMPDIList*>(GetObject(e_strName)); }
		cMPDIList*GetMPDIListByFileName(const WCHAR*e_strFileName);
		cMPDIList*GetMPDIListByFileName(const char*e_strFileName);
		cMPDIList*GetMPDIList(const char*e_strFileName,const WCHAR*e_strName);
