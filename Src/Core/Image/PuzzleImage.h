...
namespace FATMING_CORE
{
	...
	struct	sPuzzleData:public sUVData
	{
		...
		sPuzzleData(){}
		inline	sPuzzleData(WCHAR*e_strName,float*e_pfUV,POINT e_OffsetPos,POINT e_Size,POINT	e_OriginalSize,POINT e_ShowPosInPI)
		{
			ShowPosInPI = e_ShowPosInPI;
			OriginalSize = e_OriginalSize;
			swprintf(strFileName,TEMP_SIZE,L"%ls\0",e_strName);
	...
	public:
		DEFINE_TYPE_INFO();
		cPuzzleImageUnit(sPuzzleData*e_pPuzzleData,GLuint e_uiTextureIndex,cPuzzleImage*e_pPuzzleImageParent,bool e_bFetchPixels = false);
	...
	public:
	    ...
	    std::vector<std::wstring>					*m_pNameList;
		...
		void    AddNameObject(WCHAR*e_strName,int e_iIndex,float e_fTimeGap)
		{
	...
	//==========================
	class cPuzzleImage:public cBaseImage,public cObjectListByName<cPuzzleImageUnit>
	{
	    //for animation in the pi image
		GET_SET_DEC(cObjectListByName<cImageIndexOfAnimation>*,m_pImageIndexOfAnimation,GetImageIndexOfAnimationList,SetImageIndexOfAnimationList);
	...
	public:
		...
		//make sure the numerial image all have same size,
		//and it's sequence,and remember delete it after use it,
		//ensure u have no release puzzle image before delete numerial image
		cNumerialImage*	GetNumerialImageByName(WCHAR*e_strNumerImageName);
		cNumerialImage*	GetNumerialImageByName(WCHAR*e_str0ImaneName,WCHAR*e_str9ImaneName);
		float*   GetAllChildrenTriangleStripUV();
		float*   GetAllChildrenTwoTriangleUV();
		//from m_pImageIndexOfAnimation to generate a SA
		cSAList*	GetSAByImageData();
		//
		static char*       GetFileName(const WCHAR*e_strObjectName);
		static char*       GetFileName(const char*e_strObjectName);
	};
}
