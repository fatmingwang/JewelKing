...
namespace FATMING_CORE
{
	...
	cUIImage*	cImageParser::GetUIImage(const WCHAR*e_strName)
	{
		return dynamic_cast<cUIImage*>(GetObject(e_strName));
	}

	cBaseImage*	cImageParser::GetBaseImage(const WCHAR*e_strName)
	{
		return dynamic_cast<cBaseImage*>(GetObject(e_strName));
	}

	cPuzzleImage*	cImageParser::GetPuzzleImageByFileName(const WCHAR*e_strName)
	{
		std::string	l_strFileName = UT::WcharToChar(e_strName);
		WCHAR*l_strPIName = UT::GetFileNameWithoutFullPath(e_strName);
		return GetPuzzleImage(l_strFileName.c_str(),l_strPIName);
	}

	cPuzzleImage*	cImageParser::GetPuzzleImage(const WCHAR*e_strName)
	{
	...
	cPuzzleImage*	cImageParser::GetPuzzleImage(const char*e_strFileName,const WCHAR*e_strPIName)
	{
	...
	void	cImageParser::HandleElementData(TiXmlElement *e_pTiXmlElement)
	{
		...
		COMPARE_NAME("Numerial")
			m_eImageType = eIT_NUMERIAL;
		else
		...
		if(!this->m_pCurrentTiXmlElement->m_bDone)
		{
			...
			PARSE_NAME_VALUE_START
				...
				COMPARE_NAME("UV")
				{
					...
					if( l_p )
					{
						assert(l_p->GetImageHeight()==UT::power_of_two(l_p->GetImageHeight()));
						float	l_fUV[4];
						GetUV(l_strValue,l_fUV);
						l_p->SetUV(l_fUV);
						l_p->SetImageSize((int)(l_p->GetImageWidth()*(l_fUV[2]-l_fUV[0])),(int)(l_p->GetImageHeight()*(l_fUV[3]-l_fUV[0])));
					}
				}
	...
	void	cImageParser::ProcessUITexture()
	{
		if(!this->m_pCurrentTiXmlElement->m_bDone)
		{
			...
			PARSE_NAME_VALUE_START
				COMPARE_NAME("Name")
				{
					l_pUIImage = new cUIImage(StringAddDirectory(UT::WcharToChar(l_pAttribute->Value())));
				}
				else
	...
	void	cImageParser::ProcessNumerialTexture()
	{
		if(!this->m_pCurrentTiXmlElement->m_bDone)
		{
			PARSE_NAME_VALUE_START
				COMPARE_NAME("Name")
				{
					char*l_strFileName = this->StringAddDirectory(UT::WcharToChar(l_strValue));
					//l_strValue = StringAddDirectory(l_pAttribute->Value());
					//l_Value = StringAddRootDirectory(l_Value);
					cNumerialImage*l_p = new cNumerialImage(l_strFileName);
					bool	l_bSame = this->AddObject(l_p);
					if( !l_bSame )
					{
						this->m_strErrorMsg += l_strValue;
						this->m_strErrorMsg += L"loading numerial texture failed\n";
					}
				}
			PARSE_NAME_VALUE_END
		}
	...
	void	cImageParser::ProcessPuzzleImageTexture()
	{
		...
		COMPARE_ELEMENT_VALUE("PuzzleImage")
		{
			if( this->m_pCurrentTiXmlElement->m_bDone )
			{
				...
				PARSE_NAME_VALUE_START
					...
					{
						//char	l_strFileName[TEMP_SIZE];
						//sprintf(l_strFileName,"%s.png\0",StringAddDirectory(l_strValue));
						if( !g_bSkipImageLoad )
							l_pPuzzleImage = new cPuzzleImage(StringAddDirectory(UT::WcharToChar(l_strValue)),l_pPuzzleDataList,false,g_bImageLoaderForFetchPixelData);
						else
						{
							l_pPuzzleImage = new cPuzzleImage(0,l_pPuzzleDataList,false,g_bImageLoaderForFetchPixelData);
							//just don't need pixels data,but inner SimpleGLTexture is set to true if no pixels data
							l_pPuzzleImage->SetTextureFromResource(false);
						}
						l_pPuzzleImage->SetName(UT::GetFileNameWithoutFullPath(this->m_strFileName));
					...
					{//ther only should work at editor!.
						if( m_bSortPIFileAsOriginal )
						{
							...
							WCHAR*l_ImageName = wcstok((WCHAR*)l_strValue,L",");
		...
		COMPARE_ELEMENT_VALUE("PuzzleUnit")
		{
			if(!this->m_pCurrentTiXmlElement->m_bDone )
			{
				...
				//WCHAR	strFileName[TEMP_SIZE];

				sPuzzleData	l_sPuzzleData;
				PARSE_NAME_VALUE_START
					COMPARE_NAME("Name")
					{
						swprintf(l_sPuzzleData.strFileName,TEMP_SIZE,L"%ls\0",l_strValue);
					}
					else
					COMPARE_NAME("UV")
					{
						GetUV((WCHAR*)l_strValue,l_sPuzzleData.fUV);
					}
					else
		...
		COMPARE_ELEMENT_VALUE("AnimationDataUnit")
		{
		    if(!this->m_pCurrentTiXmlElement->m_bDone )
		    {
                ...
                PARSE_NAME_VALUE_START
				    ...
				    {
				        WCHAR*l_strImageName = wcstok((WCHAR*)l_strValue,L",");
	...
	void	cImageParser::Export(char*e_strFileName)
	{
		...
		for( int i=0;i<Count();++i )
		{
			...
			WCHAR	l_strWithExtendName[TEMP_SIZE];
			swprintf(l_strWithExtendName,TEMP_SIZE,L"%s.png",l_strName);

		//<Numerial Name="NumerialImage/CardNI.png" MaxU="1" />
		//<UI Name="388.png" Position="0,0" UV="0,0,1,1" />
		//<Common Name="BackGround/BKImage.png" UV="0,0,1,1" />
	...
	//</PuzzleImage>
			...
			if( !wcscmp(l_strType,cNumerialImage::TypeID) )
			{
				cNumerialImage*l_pNumerialImage = dynamic_cast<cNumerialImage*>(l_pNamedTypedObject);
				l_pXMLWriter->StartElement("Numerial");
				l_pXMLWriter->AddAttribute("MaxU",l_pNumerialImage->GetMaxU());
				l_pXMLWriter->AddAttribute("Name",UT::WcharToChar(l_strWithExtendName));
				l_pXMLWriter->EndElement();
			}
			else
			if( !wcscmp(l_strType,cPuzzleImage::TypeID) )
			{
				...
				int	l_iPIWidth = l_pPuzzleImage->GetImageWidth();
				int	l_iPIHeight = l_pPuzzleImage->GetImageHeight();
