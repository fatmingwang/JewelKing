...
namespace FATMING_CORE
{
	...
	inline	sTexBehaviorDataWithImageIndexData*	GetTexBehaviorDataWithImageIndexData(const WCHAR*e_str,sPuzzleData**e_ppPuzzleData,int e_iNumData,WCHAR*e_pstrNotExistImageName,Vector3*e_pPosOffset)
	{
		...
		WCHAR*	l_str = wcstok(l_strData,L",:");
		//size
		l_pData->Size.x = (float)_wtof(l_str);
		l_str = wcstok(0,L",:");
		l_pData->Size.y = (float)_wtof(l_str);
		l_str = wcstok(0,L",:");
		//angle
		l_pData->vAngle.z = (float)_wtof(l_str);
		l_str = wcstok(0,L",:");
		//color
		l_pData->vColor.x = _wtoi(l_str)/255.f;
		l_str = wcstok(0,L",:");
		l_pData->vColor.y = _wtoi(l_str)/255.f;
		l_str = wcstok(0,L",:");
		l_pData->vColor.z = _wtoi(l_str)/255.f;
		l_str = wcstok(0,L",:");
		l_pData->vColor.w = _wtoi(l_str)/255.f;
		l_str = wcstok(0,L",:");
		//image index
		...
		if( l_str )
		{
			l_pData->bMirror = _wtoi(l_str)?true:false;
		}
		if( l_pPuzzleData )
		{
...
inline	void	ExportPointDataList(ATG::XMLWriter*e_pXMLWriter,cCueToStartCurveWithTime*e_pCueToStartCurvesWithTime)
	...
	void	ExportMPDI(ATG::XMLWriter*e_pXMLWriter,cMPDIList*e_pMPDIList)
	{
		...
		//1.1 fix point to left not center.
		...
		for( int j=0;j<l_iNum;++j )
		{
			...
			if( l_pMultiPathDynamicImage->GetCameraView() )
				e_pXMLWriter->AddAttribute("CameraView",ValueToStringW(*l_pMultiPathDynamicImage->GetCameraView()));
			...
			e_pXMLWriter->AddAttribute("TotalPlayTime",(float)l_pMultiPathDynamicImage->GetTotalPlayTime());
			if( l_pMultiPathDynamicImage->IsStopAtLastFrame() )
				e_pXMLWriter->AddAttribute("StopAtLastFrame",1);
			...
			for( int k=0;k<l_iNumPath;++k )
			{
				...
				e_pXMLWriter->StartElement("CueToStartCurvesWithTime");
					...
					if(l_pCueToStartCurvesWithTime->IsColorBlending())
					{
						...
						e_pXMLWriter->AddAttribute("SrcBlendingData",(int)l_pCueToStartCurvesWithTime->GetSrcBlendingMode());
						e_pXMLWriter->AddAttribute("DestBlendingData",(int)l_pCueToStartCurvesWithTime->GetDestBlendingMode());
					}
	...
	void	cAnimationParser::ProcessMPDI()
	{
		COMPARE_ELEMENT_VALUE("MPDIList")
		{
			if(!this->m_pCurrentTiXmlElement->m_bDone)
			{
				...
				PARSE_NAME_VALUE_START
					...
					{
					    WCHAR*l_strPIName = 0;
					    WCHAR   l_strOriginalName[MAX_PATH];
					    ...
					    {
					        l_strPIName = (WCHAR*)l_strValue;
					    }
					    for(int i=0;i<l_iPICount;++i)
					    {
						    char*	l_strFileName = StringAddDirectory(UT::WcharToChar(l_strPIName));
			...
			if(!this->m_pCurrentTiXmlElement->m_bDone)
			{
				...
				PARSE_NAME_VALUE_START
					...
					COMPARE_NAME("TotalPlayTime")
					{
						...
						l_pCurrentMultiPathDynamicImage->SetTotalPlayTime(l_fTime);
					}
					else
					COMPARE_NAME("StopAtLastFrame")
					{
						l_pCurrentMultiPathDynamicImage->SetStopAtLastFrame(true);
					}
					else
					...
					{
						cMatrix44*l_pMat = new cMatrix44(GetMatrix(l_strValue));
						l_pCurrentMultiPathDynamicImage->SetCameraView(l_pMat);
					}
				TO_NEXT_VALUE
			}
			else
			...
			!wcscmp(m_pCurrentTiXmlElement->Value(),L"CueToStartCurvesWithTime"))
		{
			if(!this->m_pCurrentTiXmlElement->m_bDone)
			{
				...
				PARSE_NAME_VALUE_START
					...
					COMPARE_NAME("Loop")
					{
						l_pCueToStartCurvesWithTime->SetAnimationLoop(VALUE_TO_BOLLEAN);
						l_pCueToStartCurvesWithTime->SetCurveLoop(VALUE_TO_BOLLEAN);
					}
					...
					{
						l_pCueToStartCurvesWithTime->SetSrcBlendingMode((GLenum)VALUE_TO_INT);
						
					}
					COMPARE_NAME("DestBlendingData")
					{
						l_pCueToStartCurvesWithTime->SetDestBlendingMode((GLenum)VALUE_TO_INT);
					}
				TO_NEXT_VALUE
		...
		COMPARE_ELEMENT_VALUE("PointDataList")
		{
			...
			{//for new version data
				if( this->m_fMPDIVersion >= 1.f )
				{
					...
					cPuzzleImage*l_pPuzzleImage = 0;
					sPuzzleData*l_pPuzzleData = 0;
					const WCHAR*l_strPIName = 0;
					sTexBehaviorDataWithImageIndexData	l_sTexBehaviorDataWithImageIndexData;
					PARSE_NAME_VALUE_START
					COMPARE_NAME("Index")
					{
					#ifdef _DEBUG
						//int	l_iIndex = VALUE_TO_INT;
						//assert(m_pCurrentCueToStartCurvesWithTime->GetPointDataList()->size() == l_iIndex );
					#endif
					}
					else				
					COMPARE_NAME("Size")
					...
					COMPARE_NAME("Mirror")
					{
						l_sTexBehaviorDataWithImageIndexData.bMirror = VALUE_TO_BOLLEAN;
					}
					else
				...
				else
				{
					...
					WCHAR	l_strNotExistImageName[MAX_PATH];
					memset(l_strNotExistImageName,0,sizeof(WCHAR));
					bool	l_bError = false;
					int	l_iIndex = 0;
					PARSE_NAME_VALUE_START
		...
		COMPARE_ELEMENT_VALUE("HintPointList")
		{
			if(!this->m_pCurrentTiXmlElement->m_bDone)
			{
				...
				TO_NEXT_VALUE
				sHintPoint	l_HintPoint(l_iIndex,(WCHAR*)l_strEventName.c_str(),l_eAnimationEvent);
