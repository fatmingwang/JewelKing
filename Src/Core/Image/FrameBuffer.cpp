...
namespace FATMING_CORE
{
	...
	void	cFrameBuffer::StartDraw(bool e_bClearScreen)
	{
		...
		cGameApp::m_svViewPortSize.x = (float)this->m_uiWidth;
		cGameApp::m_svViewPortSize.y = (float)this->m_uiHeight;
		glBindFramebuffer(GL_FRAMEBUFFER, m_FramebufferID);
		if( e_bClearScreen )
		{
...
//		glPopAttrib();
		...
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
		cGameApp::m_svViewPortSize.x = (float)m_OriginalViewPortSize[2];
		cGameApp::m_svViewPortSize.y = (float)m_OriginalViewPortSize[3];
	...
	void	cFrameBuffer::DrawBuffer(POINT e_Pos,POINT e_Size)
	{
		...
		DrawQuadWithTextureAndColorAndCoordinate((float)e_Pos.x,(float)e_Pos.y,0.f,e_Size.x,e_Size.y,Vector4::One,l_fTextureCoordinate,0);
	}
