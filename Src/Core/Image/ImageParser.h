...
namespace FATMING_CORE
{
	...
	//=====================
	class	cImageParser:public cObjectListByName<NamedTypedObject>,public ISAXCallback
	{
	    //for animation data,jsut a temp
	    cObjectListByName<cImageIndexOfAnimation>*m_pCurrentImageIndexOfAnimationList;
	...
	public:
		...
		cUIImage*	GetUIImage(const WCHAR*e_strName);

		cBaseImage*	GetBaseImage(const WCHAR*e_strName);

		cPuzzleImage*	GetPuzzleImageByFileName(const WCHAR*e_strName);
		cPuzzleImage*	GetPuzzleImage(const WCHAR*e_strName);
		cPuzzleImage*	GetPuzzleImage(int e_iIndex);
		cPuzzleImage*	GetPuzzleImage(const char*e_strFileName,const WCHAR*e_strPIName);
