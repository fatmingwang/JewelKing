...
namespace FATMING_CORE
{
	...
	void	cVideoSubMPDI::InternalInit()
	{
		...
		{
			m_pAVIPlayer->SetLoop(this->m_bAnimationLoop);
			m_pAVIPlayer->SetCurrentFrameByTime(this->m_fStartTime);
		}
		m_vPos = m_vCurrentPosition = Vector3::Zero;
	}
	...
	{
		float	l_fPosX = this->m_vCurrentPosition.x+this->m_vPos.x;
		float	l_fPosY = this->m_vCurrentPosition.y+this->m_vPos.y;
	...
	{
		bool	l_b = m_pAVIPlayer->OpenFile(UT::CharToWchar(e_strFileName));
		if( l_b )
		{
	...
	void	cVideoSubMPDI::RenderByGlobalTime(float e_fTime,bool e_bRotateWithAngle)
	{
