#if defined USING_BULLET_PHYSICAL_ENGINE || defined (WIN32)
...
{
	eCollisionType	StringToCollisionType(const WCHAR*e_strValue)
	{
		if(!wcscmp(e_strValue,L"Circle"))
		{
			return eCT_CIRCLE;
		}
		else
	...
	c2DImageCollisionDataForEditor::sImageCollisionData::sCollisionData::sCollisionData(sCollisionData*e_pCollisionData)
	{
		...
		if( !this->bConvex )
		{
			this->pCircleData = new sCircle;
			*pCircleData = *e_pCollisionData->pCircleData;
		}
		else
		{
	...
	bool	c2DImageCollisionDataForEditor::sImageCollisionData::sCollisionData::Collide(float e_fRadius,float* e_pfMatrix)
	{
		...
		else
		{
			if( pCircleData )
			{
				cMatrix44	l_mat = cMatrix44::TranslationMatrix(pCircleData->vPos);
				if(SphereCollideSphere(e_pfMatrix,e_fRadius,l_mat,pCircleData->fRadius))
					return true;
			}
		}
		return false;
	...
	void	c2DImageCollisionDataForEditor::sImageCollisionData::sCollisionData::Render(Vector4 e_vColor)
	{
		...
		if(this->bConvex)
		{
			RenderLine(&ConvexHullPointData,e_vColor,l_mat);
		}
		else
		{
			if( pCircleData )
			{
				sCircle l_Circle = *pCircleData;
				cMatrix44	l_matTranslation = cMatrix44::TranslationMatrix(l_Circle.vPos);
				l_mat = l_mat*l_matTranslation;
				//l_mat.SetTranslation(l_mat.GetTranslation()+l_matTranslation.GetTranslation());
				l_Circle.vPos.x = l_mat.r[3].x;
				l_Circle.vPos.y = l_mat.r[3].y;
					RenderCircle(Vector2(l_Circle.vPos.x,l_Circle.vPos.y),l_Circle.fRadius,e_vColor);
			}
		}
	}
	...
	bool	c2DImageCollisionDataForEditor::Collide(float e_fRadius,float *e_pfMatrix)
	{
		...
		for( int i=0;i<(int)this->m_pCurrentImageCollisionData->CollisionDataVector.size();++i )
		{
			...
			{
				sCircle*l_pCircle = m_pCurrentImageCollisionData->CollisionDataVector[i]->pCircleData;
				Vector3	l_vPos = l_pCircle->vPos;
				l_pbtCollisionShape = new cbtSphereShape(l_pCircle->fRadius,l_pCircle->vPos);
			}
	...
	void	c2DImageCollisionDataForEditor::MouseDown(POINT e_Pos,int e_iMode)
	{
		...
		if( m_pCurrentImageCollisionData && m_pCurrentImageCollisionData->iCollisionDataIndex != -1 )
		{
			...
			{
				if(!m_pCurrentImageCollisionData->pCurrentCollisionData->pCircleData)
				{
					m_pCurrentImageCollisionData->pCurrentCollisionData->pCircleData = new sCircle();
					m_pCurrentImageCollisionData->pCurrentCollisionData->pCircleData->fRadius = 12.f;
					m_pCurrentImageCollisionData->pCurrentCollisionData->pCircleData->vPos = l_vPos;
				}
				else
				{
					m_pCurrentImageCollisionData->pCurrentCollisionData->pCircleData->vPos = l_vPos;
	...
	void	c2DImageCollisionDataForEditor::MouseMove(POINT e_Pos,int e_iMode)
	{
		if( m_bMouseDown )
		{
			if( m_pCurrentImageCollisionData && m_pCurrentImageCollisionData->iCollisionDataIndex != -1 )
			{
				...
				if( m_pCurrentImageCollisionData->pCurrentCollisionData->pCircleData )
				{
					m_pCurrentImageCollisionData->pCurrentCollisionData->pCircleData->vPos = Vector3((float)e_Pos.x,(float)e_Pos.y,0.f);
				}
			}
		}
	}
	...
	void	c2DImageCollisionDataForEditor::Export(char*e_strFileName,cPuzzleImage*e_pPuzzleImage)
	{
		...
		for( UINT i=0;i<l_iuiNumImageCollisionData;++i )
		{
			...
			for( UINT j=0;j<l_uiNumCollisionData;++j )
			{
				...
				{
					if( l_pCollisionData->pCircleData )
					{
						l_XMLWriter.AddAttribute("Type","Circle");
						l_XMLWriter.AddAttribute("DataCount",1);
						l_XMLWriter.AddAttribute("Data",ValueToString(l_pCollisionData->pCircleData->vPos));
						l_XMLWriter.AddAttribute("Radius",l_pCollisionData->pCircleData->fRadius);
					}
					else
					{
	...
	{
		const WCHAR*l_strValue = this->m_pCurrentTiXmlElement->Value();
	...
	void	c2DImageCollisionDataForEditor::ProcessImageCollisionData()
	{
		...
		{//fuck I am lazy to debug fuck!
			...
			PARSE_NAME_VALUE_START
				COMPARE_NAME("Name")
				{
					swprintf(m_strForCollidedpiFile,MAX_PATH,L"%ls\0",l_strValue);
				}
				else
				COMPARE_NAME("Count")
				{
...
//        <Shape Type="Circle" DataCount="1" Data="194.00,122.00,0.00," />
...
//        <Shape Type="Empty" />
	void	c2DImageCollisionDataForEditor::ProcessShapeData()
	{
		if(!this->m_pCurrentTiXmlElement->m_bDone)
		{
			...
			{
				case eCT_CIRCLE:
					l_pCollisionData->pCircleData = new sCircle();
					l_pCollisionData->pCircleData->vPos = GetVector3(l_strData);
					l_pCollisionData->pCircleData->fRadius = l_fRadius;
					l_pCollisionData->bConvex = false;
					break;
				case eCT_CONVEX:
					l_pCollisionData->ConvexHullPointData = StringToVector3Vector((WCHAR*)l_strData,l_iCount);
...
//        <Shape Type="Empty" />
	void	c2DImageCollisionData::ProcessShapeData()
	{
		...
		switch(l_eCollisionType)
		{
			...
			case eCT_CONVEX:
				{
					std::vector<Vector3>l_vPos = StringToVector3Vector((WCHAR*)l_strData,l_iCount);
