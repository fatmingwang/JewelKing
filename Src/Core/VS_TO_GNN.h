...
//        inline  float _wtof( const WCHAR* e_str )
//        {
//            return _wtof( (WCHAR*)e_str );
//        }
...
//}
	...
	inline double _wtof( const WCHAR* e_str )
	{
            bool bMinus = false;
            if (*e_str == '+')
            {
...
*  If str is NULL it crashes, as the native function does.
*/
	inline int _wtoi( WCHAR* str )
	{
		return _wtol(str);
	}

	inline int _wtoi( const WCHAR* str )
	{
		return _wtol((WCHAR*)str);
	}

	inline	void	_itow(int val,WCHAR*e_strBuffer,int base)
	{
		static WCHAR buf[32] = {0};
		memset(buf,0,sizeof(WCHAR)*30);
		if( val!=0 )
		{
			int i = 30;
			for(; val && i ; --i, val /= base)
				buf[i] = L"0123456789abcdef"[val % base];
			memcpy(e_strBuffer,&buf[i+1],sizeof(WCHAR)*32-i-1);
		}
		else
		{
			e_strBuffer[0] = L'0';
