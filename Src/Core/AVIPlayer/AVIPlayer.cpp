...
#include "../Image/glLoadImage.h"
//#pragma comment(lib,"ole32.lib")
//#define	MAX_PIXELS	512
#ifndef __IPHONE__//iphone platform
//LU	RU
//
//LD	RD
...
//	DrawDibClose(m_hdc);										// Closes The DrawDib Device Context
	CloseFile();
	//DrawDibClose(m_hdc);										// Closes The DrawDib Device Context
	OutputDebugString(L"FUCK I am lazy to debug this...why it occur crash DrawDibClose?");
	m_hdc = 0;
}
...
void	cAVIPlayer::ParseVideoStream()
{
	...
	m_hBitmap = CreateDIBSection (m_hdc, (BITMAPINFO*)(&m_Bmih), DIB_RGB_COLORS, (void**)(&m_pDIBData), NULL, NULL);
...
bool cAVIPlayer::OpenFile(WCHAR*e_strFileName)
//void cAVIPlayer::OpenFile(WCHAR*e_strFileName)				// Opens An AVI File (szFile)
{
	//CoInitialize(NULL);
	CloseFile();
	if( m_uiTex_id == 0 )
...
void	cAVIPlayer::Render(float e_fPosX,float e_fPosY,POINT e_Size)
{
	if( !m_PGF )
		return;
	cSimpleGLTexture::m_suiLastUsingImageIndex = m_uiTex_id;
	glBindTexture( GL_TEXTURE_2D, m_uiTex_id);
	//glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);	// Set Texture Max Filter
	//glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);	// Set Texture Min Filter
	//glTexGeni(GL_S, GL_TEXTURE_GEN_MODE, GL_SPHERE_MAP);				// Set The Texture Generation Mode For S To Sphere Mapping
	...
	//glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, m_iWidth, m_iHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, m_pDIBData);
	float	l_UV[] = {0,1,1,0};
	DrawQuadWithTextureAndColorAndCoordinate(e_fPosX,e_fPosY,0.f,e_Size.x,e_Size.y,Vector4::One,l_UV,0.f);
...
bool InitAVIData(void);
bool FreeAVIData(void);
bool ExtractAVIFrame(LONG pos);
void FourCCToString(DWORD code, char* buffer);
