...
public:
	...
	void	EvaluateLocalXForm(float e_fTime);
	bool	m_bAnimation;
#ifdef _DEBUG
...
class	cAnimationMesh:public cMesh
{
	...
	GETP_SET_DEC(cObjectListByName<cBone>,m_AllBoneVector,GetAllBoneVector,SetAllBoneVector);
	//the bone for skinning,do not delete its vector data because its come from m_AllBoneVector
	GETP_SET_DEC(cObjectListByName<cBone>,m_SkinningBoneVector,GetSkinningBoneVector,SetSkinningBoneVector);
//animation time
	GET_SET_DEC(float,m_fMinKeyTime,GetMinKeyTime,SetMinKeyTime);
	GET_SET_DEC(float,m_fMaxKeyTime,GetMaxKeyTime,SetMaxKeyTime);
