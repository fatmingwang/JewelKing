...
void	cColladaParser::ProcessController()
{
	if( !m_pAllAnimationMesh )
		m_pAllAnimationMesh = new cObjectListByName<cAnimationMesh>;
	...
	for( UINT i=0;i<l_iNumInstanceNode;++i )
	{
		...
		if( l_pInstance->IndtanceType == sVisualSceneLibrary::sNode::eIT_CONTROLLER )//find skeleton data
		{
			...
			{
#ifdef WIN32
				OutputDebugString(L"fatal error!!! void	cColladaParser::ProcessController()");
#endif
				continue;
			}
			cMesh*l_pMesh = 0;
			...
			cObjectListByName<cBone>*l_pAllBoneList = new cObjectListByName<cBone>;
			...
			//cObjectListByName<cBone>*l_pAllBoneList = &l_pSkeletonNode->m_AllBoneData;
			cObjectListByName<cBone> l_BoneListByOrder;//have to take a order by name array,so joints influnce index would be righ
			if( l_pAllBoneList->Count() == 0 )
			{
				...
				if( l_pMesh )
				{
					...
					else
					{
						...
						OutputDebugString(L" is not exists!!");
#endif
					}
				}
				continue;
			...
			{
				WCHAR*l_strName = l_pJointsSource->strAllJointName[k];
			...
			//				OutputDebugString(l_pCurrentBoneForDelete->GetName());
			//				OutputDebugString(L"        !!!WHAT??fuck....why artist make this dummy joints?\n");
			...
			l_BoneListByOrder[0]->SetNextibling(0);//if nex sibling is exiss,it's unnecessary.
