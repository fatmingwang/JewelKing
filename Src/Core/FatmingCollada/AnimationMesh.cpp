...
void	cBone::SetFormKeyFrames(FloatTocMatrix44Map	e_FormKeyFrames)
{
	...
	else
	{
		...
		OutputDebugString(L"animation frame is zero");
	}
#endif
#endif
}
...
//
//========================================================
void	cBone::EvaluateLocalXForm(float timeValue)
{
	...
	else
	{
		...
		Vector3 tNew = t0 + s * (t1 - t0);
		mNew.SetTranslation(tNew);
	}
	mNew = mNew.Transposed();
	this->SetLocalTransform(mNew);
}
...
void	cAnimationMesh::RefreshAnimationData()
{
	...
	if( l_bShowDebugInfo )
	{
		...
		{
			//OutputDebugString(m_SkinningBoneVector[i]->GetName());
			OutputDebugString(m_SkinningBoneVector[i]->m_strSID.c_str());
			OutputDebugString(L"\n");
		}
	}
#endif
...
void	cAnimationMesh::Render()
{
	...
	{
		this->m_pVBOBuffer->RenderByShader();
	}
}
...
void	cAnimationMesh::RenderSkeleton()
{
	...
	//l_mat = cMatrix44::Identity;
	
	//if(this->m_Sphere.Radius<1.f)
	{
		//float	l_fScale = 1.f/m_Sphere.Radius;
		//l_mat *= cMatrix44::ScaleMatrix(Vector3(l_fScale,l_fScale,l_fScale));
	}
	//SetupShaderWorldMatrix(l_mat);
	GLRender::RenderLine((float*)&l_vAllVertices[0],l_iBoneSize*2,Vector4(0,1,0.5f,1),3,l_mat);
	GLRender::RenderPoints(l_vAllVertices,l_iBoneSize*2,15,Vector4(0,1,0.5f,1),l_mat);
}