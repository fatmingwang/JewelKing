...
int		sController::sSkin::VisualSceneJointNodeNameToIndex(const WCHAR*e_strNodeName)
{
	...
	{
		if(!wcscmp(l_pJointSource->strAllJointName[i],e_strNodeName))
			return i;
	}
	return -1;
}
...
void	cLibraryControllers::Parsecontroller_skin_source_Name_array(TiXmlElement*e_pTiXmlElement)
{
	...
	const WCHAR*l_strText = e_pTiXmlElement->GetText();
	assert(l_strText);
	m_pCurrentSource->strAllJointName = new WCHAR*[m_pCurrentSource->iCount];
	WCHAR*	l_strBoneName = wcstok((WCHAR*)l_strText,L" ");
	int	l_iIndex = 0;
	while( l_strBoneName )
	{
		m_pCurrentSource->strAllJointName[l_iIndex] = new WCHAR[wcslen(l_strBoneName)+1];
		swprintf(m_pCurrentSource->strAllJointName[l_iIndex],MAX_PATH,L"%ls\0",l_strBoneName);
		l_iIndex++;
