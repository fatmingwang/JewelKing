	...
	for(UINT l_iNumInstance = 0;l_iNumInstance<l_iInstanceNode;++l_iNumInstance)
	{
		...
		for(UINT l_iNumMesh = 0;l_iNumMesh<l_pColladaGeomatries->pGeometryMesh->AllMeshIndexList.size();++l_iNumMesh)
		{
			...
			for(UINT m=0;m<l_pComonEffect->AllTechniqueAndTextureName.size();++m)
			{
				...
				cSimpleGLTexture*l_pSimpleGLTexture = this->m_LibraryImages.m_ImageList.GetObject(l_strTextureName);
				if( !l_pSimpleGLTexture )
				{
					WCHAR	*l_strTechniqueTextureName = wcstok((WCHAR*)l_pTechniqueData->strTextureName,L"-");
					l_pSimpleGLTexture = this->m_LibraryImages.m_ImageList.GetObject(l_strTextureName);
					if( !l_pSimpleGLTexture )
					{//for FCollada export
						...
						if( l_iLastDashIndex != -1 )
						{
							...
							l_pSimpleGLTexture = this->m_LibraryImages.m_ImageList.GetObject(l_strTechniqueTextureName);
						}
						if( !l_pSimpleGLTexture )
						{
#ifdef WIN32
				...
				if(l_pMesh->GetObject(l_pSimpleGLTexture->GetName()))
					continue;
				//assign it to our mesh
				l_pSimpleGLTexture->SetTextureFromResource(true);
				sTechniqueAndTexture*l_psTechniqueAndTexture = new sTechniqueAndTexture(l_pSimpleGLTexture,l_pTechniqueData->eTL);
				l_psTechniqueAndTexture->SetTextureFromResource(false);
			...
			if( l_pMesh->Count() == 0 )
			{
				if( !l_pEffectData->m_strName )
					continue;
				cSimpleGLTexture*l_pSimpleGLTexture = this->m_LibraryImages.m_ImageList.GetObject(l_pEffectData->m_strName);
				if( !l_pSimpleGLTexture )
				{
					continue;
				}
				//assign it to our mesh
				l_pSimpleGLTexture->SetTextureFromResource(true);
				sTechniqueAndTexture*l_psTechniqueAndTexture = new sTechniqueAndTexture(l_pSimpleGLTexture,eTL_DIFFUSE);
				l_psTechniqueAndTexture->SetTextureFromResource(false);
				l_pMesh->AddObject(l_psTechniqueAndTexture);
			}
		}
