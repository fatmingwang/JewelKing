...
bool IsEquivalent(const char* sz1, const char* sz2) { return sz1 == sz2 || (sz1 != NULL && sz2 != NULL && strcmp(sz1, sz2) == 0); }
bool IsEquivalent(const std::string& sz1, const char* sz2) { return sz2 != NULL && strcmp(sz1.c_str(), sz2) == 0; }
bool IsEquivalent(const char* sz1, const std::string& sz2) { return sz1 != NULL && strcmp(sz1, sz2.c_str()) == 0; }
bool IsEquivalent(const std::string& sz1, const std::string& sz2) { return strcmp(sz1.c_str(), sz2.c_str()) == 0; }
...
namespace FUDaeInterpolation
{
	Interpolation FromString(const std::string& value)
	{
		...
		else if (value == DAE_NURBS_INTERPOLATION) return NURBS;
	...
	const char* ToString(const Interpolation& value)
	{
		...
		case NURBS: return DAE_NURBS_INTERPOLATION;
		case UNKNOWN:
...
namespace FUDaeNURBSSurface
{
	...
	{
		if (IsEquivalent(str, DAE_VERTEX_NURBS_SURF_TYPE)) return VERTEX;
		else if (IsEquivalent(str, DAE_TEXTURE_NURBS_SURF_TYPE)) return TEXTURE;
		else if (IsEquivalent(str, DAE_NORMAL_NURBS_SURF_TYPE)) return NORMAL;
		else return UNKNOWN;
	}
	...
	const char* ToString(Type type)
	{
		switch (type)
		{
		case VERTEX: return DAE_VERTEX_NURBS_SURF_TYPE;
		case TEXTURE: return DAE_TEXTURE_NURBS_SURF_TYPE;
		case NORMAL: return DAE_NORMAL_NURBS_SURF_TYPE;
		default: return DAE_UNKNOWN_NURBS_SURF_TYPE;
		}
	}
};
#endif // PREMIUM_NURBS
...
{	
	...
	{
		if (IsEquivalent(value, DAEFC_COLOR_BUFFER)) return COLOR;
		else if (IsEquivalent(value, DAEFC_DEPTH_BUFFER)) return DEPTH;
		else if (IsEquivalent(value, DAEFC_STENCIL_BUFFER)) return STENCIL;
		else return INVALID;
	}
	...
	const char* ToString(RenderTargetBuffer state)
	{
		switch (state)
		{
		case COLOR: return DAEFC_COLOR_BUFFER;
		case DEPTH: return DAEFC_DEPTH_BUFFER;
		case STENCIL: return DAEFC_STENCIL_BUFFER;
		case INVALID:
		default: return DAEERR_UNKNOWN_ELEMENT;
		}
	}
