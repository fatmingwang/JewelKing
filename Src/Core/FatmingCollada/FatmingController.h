	...
	struct	sController
	{
		struct	sSkin
		{
			...
			struct	sSource
			{
				...
				WCHAR**strAllJointName;	//for skin joint,reference to visual scene joints node.
				float*pData;			//for weights
				cMatrix44	*pMatrcies;	//for bind pose
				sSource()
				{
					memset(this,0,sizeof(sSource));
				}
				~sSource()
				{
					if( strAllJointName )
					{
						for( int i=0;i<iCount;++i )
							SAFE_DELETE(strAllJointName[i]);
						SAFE_DELETE(strAllJointName);
					}
					SAFE_DELETE(pData);
