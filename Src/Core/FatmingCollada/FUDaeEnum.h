/*
	...
	Portions of the code may be:
	Copyright (C) 2005-2007 Sony Computer Entertainment America.
    Copyright (C) 2004-2006 Autodesk Media Entertainment.
...
bool IsEquivalent(const std::string& sz1, const char* sz2); /**< See above. */
bool IsEquivalent(const char* sz1, const std::string& sz2); /**< See above. */
bool IsEquivalent(const std::string& sz1, const std::string& sz2); /**< See above. */
...
namespace FUDaeInterpolation
{
	...
	enum Interpolation
	{
		...
		NURBS,/**< Nurbs interpolation. Uses weights and knots. */
...
namespace FUDaeSplineForm
{
	...
	Form FromString(const std::string& value);
...
#ifdef PREMIUM_NURBS
/** Contains the NURBS surface types and their conversion functions. */
namespace FUDaeNURBSSurface
{
	/** A surface type. */
	enum Type
	{
		VERTEX = 0, /**< A position surface. This is the default surface type. */
		TEXTURE, /**< A texture surface. */
		NORMAL, /**< A geometric normal surface. */

		UNKNOWN, /**< An unknown surface type. */
		DEFAULT = VERTEX,
	};

	/** Converts a FCollada surface type string into a surface type.
		@param str The FCollada surface type string.
		@return The surface type. */
	Type FromString(const std::string& str);

	/** Converts a surface type into its FCollada surface type string.
		@param type The surface type.
		@return The surface type string. */
...
namespace FUDaeTextureChannel
{
	...
	Channel FromString(const std::string& value);
};

/** Contains the texture wrap modes.*/
namespace FUDaeTextureWrapMode
{
	/** The texture wrap mode. Also called repeat mode in some SDKs. */
	enum WrapMode
	{
		NONE = 0, /**< No wrap mode. Only texture coordinates in the [0,1] range are valid. The behavior for the other texels remains undefined. */
		WRAP, /**< Repeat wrap mode. Texture coordinates outside the [0,1[ have their integer component subtracted out to bring them in the [0,1[ range. */
		MIRROR, /**< Mirror wrap mode. Texture coordinates outside the [0,2[ have their even component subtracted out to bring them in the [0,2[ range. Texture coordinates in the [1,2[ range are inverted and brought to the ]1,0] range to be contained within the texture. */
		CLAMP, /**< Clamp wrap mode. All texture coordinates less than zero are forced to zero and all texture coordinates greater than one and forced to one, so that all texture coordinates are in the [0,1] range. */ 
		BORDER, /**< Border wrap mode. All texture coordinates outside of the [0,1] range have a provided color value that is not related to the texture. */

		UNKNOWN, /**< An unknown wrap mode. Usually implies a failed string->enum conversion. */
		DEFAULT = WRAP, /**< The default wrap mode. Used for all unassigned values. */
	};
...
{
	/** The texture filtering functions. */
	enum FilterFunction
	{
		NONE = 0, /**< No texture filtering is applied. Use NEAREST instead. */
		NEAREST, /**< Point texture filtering. The closest texel is applied fully. */
		LINEAR, /**< Linear texture filtering. Four texels are read and linearly interpolated to generate a color value. */
		NEAREST_MIPMAP_NEAREST, /**< Point-point texture filtering. The closest texel of the closest mip-level is applied fully. */
		LINEAR_MIPMAP_NEAREST, /**< Linear-point texture filtering. Four texels from the closest mip-level are read and linearly interpolated to generate a color value. */
		NEAREST_MIPMAP_LINEAR, /**< Point-linear texture filtering. Two texels from the two closest mip-levels are read and linearly interpolated to generate a color value. */
		LINEAR_MIPMAP_LINEAR, /**< Linear-linear texture filtering. Eight texels from the two closest mip-levels are read and linearly interpolatied to generate a color value. */

		UNKNOWN, /**< An unknown texture filter. This implies a failed string->enum conversion. */
		DEFAULT = NONE, /**< The default texture filter. This is used for unassigned values. */
	};
...
namespace FUDaeGeometryInput
{
	...
	typedef std::vector<Semantic> SemanticList; /**< A dynamically-sized array of geometry source data types. */
...
namespace FUDaePassState
{
	...
	enum State
	{
		...
		LIGHT_AMBIENT, /**< { uint8 lightIndex = 0, FMVector4F ambientColor = [0,0,0,1] } */
		LIGHT_DIFFUSE, /**< { uint8 lightIndex = 0, FMVector4F diffuseColor = [0,0,0,0] } */
		LIGHT_SPECULAR, /**< { uint8 lightIndex = 0, FMVector4F specularColor = [0,0,0,0] } */
		LIGHT_POSITION, /**< { uint8 lightIndex = 0, FMVector4F position = [0,0,1,0] } */
		...
		LIGHT_SPOT_DIRECTION, /**< { uint8 lightIndex = 0, FMVector4F direction = [0,0,-1] } */
		...
		TEXTURE_ENV_COLOR, /**< { uint8 textureIndex = 0, FMVector4F environmentColor = [0,0,0,0] } */
		TEXTURE_ENV_MODE, /**< { uint8 textureIndex = 0, char environmentMode[255] = "" } */

		CLIP_PLANE = 45, /**< { uint8 planeIndex = 0, FMVector4F planeValue = [0,0,0,0] } */
		CLIP_PLANE_ENABLE, /**< { uint8 planeIndex = 0, bool enabled = false } */
		BLEND_COLOR, /**< { FMVector4F blendColor = [0,0,0,0] } */
		CLEAR_COLOR, /**< { FMVector4F clearColor = [0,0,0,0] } */
		...
		DEPTH_BOUNDS, /**< { FMVector2F depthBounds = [0,1] } */
		...
		FOG_COLOR, /**< { FMVector4F fogColor = [0,0,0,0] } */
		LIGHT_MODEL_AMBIENT, /**< { FMVector4F ambientColor = [0.2,0.2,0.2,1] } */
		...
		MATERIAL_AMBIENT = 63, /**< { FMVector4F ambientColor = [0.2,0.2,0.2,1] } */
		MATERIAL_DIFFUSE, /**< { FMVector4F diffuseColor = [0.8,0.8,0.8,1] } */
		MATERIAL_EMISSION, /**< { FMVector4F emissionColor = [0,0,0,1] } */
		MATERIAL_SHININESS, /**< { float shininessOrSpecularLevel = 0.0f } */
		MATERIAL_SPECULAR, /**< { FMVector4F specularColor = [0,0,0,1] } */
		MODEL_VIEW_MATRIX, /**< { FMMatrix44 modelViewMatrix = FMMatrix44::Identity } */
		POINT_DISTANCE_ATTENUATION, /**< { FMVector3F attenuation = [1,0,0] } */
		...
		SCISSOR, /**< { FMVector4F scissor = [0,0,0,0] } */
...
/** Character-based definition of the various accessors. */
namespace FUDaeAccessor
{
	/** Common accessor type string arrays.
		These are NULL-terminated and commonly used for import/export functionality. */
	extern const char* XY[3]; /**< Use for tangents and other 2D values. */
	extern const char* XYZW[5]; /**< Use for 3D/4D vector and position sources. */
	extern const char* RGBA[5]; /**< Use for color sources. */
	extern const char* STPQ[5]; /**< Use for texture coordinate sources. */
}

/** The dimension types. */
namespace FUDaeDimensionType
{
	/** The dimension types. */
	enum DimensionType
	{
		LINE = 0, /** A linear space has one dimension. */
		PLANE, /** A planar space has two dimension. */
		VOLUME, /** A volume space has three dimension. */
		CUBE, /** A cube map is six planar spaces. */

		COUNT, /** The number of types of dimensions. */
		INVALID /** An invalid surface type. */
	};
}

/** The different buffers on a render target. */
namespace FUDaeRenderTargetBuffer
{
	/** The different buffers on a render target. */
	enum RenderTargetBuffer
	{
		COLOR = 0,	/**< The color buffer. */
		DEPTH,		/**< The depth buffer. */
		STENCIL, 	/**< The stencil buffer. */

		COUNT,		/**< The number of possible buffers on a render target. */
		INVALID		/**< An invalid buffer. */
	};

	/** Converts the FCOLLADA render target buffer name string to the render target buffer enumeration type.
		@param value The render target buffer name string.
		@return The render target buffer enumeration type. */
	RenderTargetBuffer FromString(const char* value);

	/** Converts the render target buffer enumeration type to its FCOLLADA buffer name string.
		@param state The render target buffer enumeration type.
		@return The render target buffer name string. */
	const char* ToString(RenderTargetBuffer state);
}
