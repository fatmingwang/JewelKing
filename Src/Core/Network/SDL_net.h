...
#endif
extern int SDLNet_ResolveHost(IPaddress *address, const char *host, unsigned short port);
...
*/
extern const char * SDLNet_ResolveIP(const IPaddress *ip);

/* Get the addresses of network interfaces on this system.
   This returns the number of addresses saved in 'addresses'
 */
extern int SDLNet_GetLocalAddresses(IPaddress *addresses, int maxcount);

/***********************************************************************/
/* TCP network API                                                     */
/***********************************************************************/
//ready and iServerFlag should be bool,but c language do not has boolean
struct _TCPsocket {
	int ready;
	SOCKET channel;
...
*/
extern UDPsocket SDLNet_UDP_Open(unsigned short port);

/* Set the percentage of simulated packet loss for packets sent on the socket.
*/
extern void SDLNet_UDP_SetPacketLoss(UDPsocket sock, int percent);
...
*/
extern int SDLNet_CheckSockets(SDLNet_SocketSet set, unsigned int timeout);
...
extern void SDLNet_Write16(unsigned short value, void *area);
extern void SDLNet_Write32(unsigned int value, void *area);

/* Read a 16/32 bit value from network packet buffer */
extern unsigned short SDLNet_Read16(void *area);
extern unsigned int SDLNet_Read32(void *area);
...
#define SDLNet_Write16(value, areap)	\
	(*SDL_reinterpret_cast(unsigned short *, areap) = SDL_SwapBE16(value))
#else
#if SDL_BYTEORDER == SDL_BIG_ENDIAN
#define SDLNet_Write16(value, areap)	\
do 					\
{					\
	unsigned char *area = SDL_reinterpret_cast(unsigned char *, areap);	\
	area[0] = (value >>  8) & 0xFF;	\
	area[1] =  value        & 0xFF;	\
} while ( 0 )
#else
...
#define SDLNet_Write32(value, areap) 	\
	*SDL_reinterpret_cast(unsigned int *, areap) = SDL_SwapBE32(value);
#else
#if SDL_BYTEORDER == SDL_BIG_ENDIAN
#define SDLNet_Write32(value, areap) 	\
do					\
...
#define SDLNet_Read16(areap) 		\
	(SDL_SwapBE16(*SDL_reinterpret_cast(unsigned short *, areap)))
#else
#if SDL_BYTEORDER == SDL_BIG_ENDIAN
#define SDLNet_Read16(areap) 		\
	(((SDL_reinterpret_cast(unsigned char *, areap))[0] <<  8) | (SDL_reinterpret_cast(unsigned char *, areap))[1] <<  0)
#else
#define SDLNet_Read16(areap) 		\
	(((SDL_reinterpret_cast(unsigned char *, areap))[1] <<  8) | (SDL_reinterpret_cast(unsigned char *, areap))[0] <<  0)
#endif
#endif /* !SDL_DATA_ALIGNED */
...
#define SDLNet_Read32(areap) 		\
	(SDL_SwapBE32(*SDL_reinterpret_cast(unsigned int *, areap)))
#else
#if SDL_BYTEORDER == SDL_BIG_ENDIAN
#define SDLNet_Read32(areap) 		\
	(((SDL_reinterpret_cast(unsigned char *, areap))[0] << 24) | ((SDL_reinterpret_cast(unsigned char *, areap))[1] << 16) | \
	 ((SDL_reinterpret_cast(unsigned char *, areap))[2] <<  8) |  (SDL_reinterpret_cast(unsigned char *, areap))[3] <<  0)
#else
#define SDLNet_Read32(areap) 		\
	(((SDL_reinterpret_cast(unsigned char *, areap))[3] << 24) | ((SDL_reinterpret_cast(unsigned char *, areap))[2] << 16) | \
	 ((SDL_reinterpret_cast(unsigned char *, areap))[1] <<  8) |  (SDL_reinterpret_cast(unsigned char *, areap))[0] <<  0)
#endif
#endif /* !SDL_DATA_ALIGNED */
