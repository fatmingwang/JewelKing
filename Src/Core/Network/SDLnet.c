...
int SDLNet_ResolveHost(IPaddress *address, const char *host, unsigned short port)
{
	int retval = 0;
...
void   SDLNet_Write32(unsigned int value, void *areap)
{
	*(unsigned int *)(areap) = SDL_SwapBE32(value);
}
...
#undef SDLNet_Read32
unsigned int SDLNet_Read32(void *areap)
{
	return (SDL_SwapBE32(*(unsigned int *)(areap)));
}
