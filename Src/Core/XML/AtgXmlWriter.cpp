...
VOID XMLWriter::Initialize( const CHAR* strFileName )
{
    ...
    m_hFile = CreateFile( strName, FILE_WRITE_DATA, 0, NULL, CREATE_ALWAYS, FILE_FLAG_SEQUENTIAL_SCAN, NULL );
    m_strNameStack[0] = '\0';
    m_strNameStackTop = m_strNameStack;
...
BOOL XMLWriter::StartElement( const WCHAR* strName )
{
	return StartElement(UT::WcharToChar(strName));
}
//----------------------------------------------------------------------------------
// Name: EndElement
...
{
    CHAR strTemp[4096];
    WideCharToMultiByte( CP_ACP, 0, wstrValue, (INT)wcslen( wstrValue ) + 1, strTemp, 4096, NULL, NULL );
