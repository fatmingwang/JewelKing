...
FILE* TiXmlFOpen( const char* filename, const char* mode )
{
		...
		FILE* fp = 0;
		//errno_t err = fopen_s( &fp, filename, "rb+,ccs=UNICODE" );
		errno_t err = fopen_s( &fp, filename, mode );
		//errno_t err = _wfopen_s( &fp, UT::CharToWchar(filename), L"rb+,ccs=UNICODE" );
		//clear system file not exist error message
		UT::SystemErrorCheck();
		if ( !err && fp )
			return fp;
		return 0;
...
void TiXmlBase::EncodeString( const TIXML_STRING& str, TIXML_STRING* outString )
{
	...
	while( i<(int)str.length() )
	{
		...
		else if ( c < 32 )
		{
			...
			WCHAR buf[ 32 ];
			swprintf( buf, sizeof(buf), L"&#x%02X;", (unsigned) ( c & 0xff ) );
		...
		else
		{
			...
			*outString += (WCHAR) c;	// somewhat more efficient function call.
			++i;
		}
	}
}
...
const TiXmlNode* TiXmlNode::FirstChild( const WCHAR* _value ) const
{
...
const TiXmlNode* TiXmlNode::LastChild( const WCHAR* _value ) const
{
	const TiXmlNode* node;
...
const TiXmlNode* TiXmlNode::IterateChildren( const WCHAR* val, const TiXmlNode* previous ) const
{
	if ( !previous )
	{
...
const TiXmlNode* TiXmlNode::NextSibling( const WCHAR* _value ) const
{
	const TiXmlNode* node;
...
const TiXmlNode* TiXmlNode::PreviousSibling( const WCHAR* _value ) const
{
	const TiXmlNode* node;
...
const TiXmlElement* TiXmlNode::FirstChildElement( const WCHAR* _value ) const
{
	const TiXmlNode* node;
...
const TiXmlElement* TiXmlNode::NextSiblingElement( const WCHAR* _value ) const
{
	const TiXmlNode* node;
...
void TiXmlElement::SetAttribute( const WCHAR* name, int val )
{
	WCHAR buf[64];
	swprintf( buf, sizeof(buf), L"%d", val );
...
void TiXmlElement::SetDoubleAttribute( const WCHAR* name, double val )
{
	WCHAR buf[256];
...
void TiXmlElement::SetAttribute( const WCHAR* cname, const WCHAR* cvalue )
{
    #ifdef TIXML_USE_STL
	TIXML_STRING _name( cname );
	TIXML_STRING _value( cvalue );
	#else
	const WCHAR* _name = cname;
	const WCHAR* _value = cvalue;
	#endif
	...
	TiXmlAttribute* attrib = new TiXmlAttribute( (WCHAR*)cname, cvalue );
	if ( attrib )
	{
...
const WCHAR* TiXmlElement::GetText() const
{
...
bool TiXmlDocument::LoadFile( const WCHAR* _filename, TiXmlEncoding encoding )
{
	...
	char	l_strFileName[MAX_PATH];
	memset(l_strFileName,0,sizeof(char)*MAX_PATH);
	WCHAR_TO_CHAR(_filename,l_strFileName)
	FILE* file = TiXmlFOpen( l_strFileName, "rb" );
...
bool TiXmlDocument::LoadFile( FILE* file, TiXmlEncoding encoding )
{
	...
	std::wstring data;
	data.reserve( length );
    //int l_iSize = sizeof(WCHAR);
	WCHAR* buf = new WCHAR[ length+1 ];//assume all file is unicode
	memset(buf,0,sizeof(WCHAR)*length+1);
	buf[0] = 0;
	...
	//if( buf[0] != 0xfeff )//this is ansii file,convert ansii file to unicode
	{
		...
		int	l_iNewBufferSize = strlen(l_strAnsii);
		WCHAR* pNewbuf = new WCHAR[ l_iNewBufferSize+1 ];
		SystemErrorCheck();
		mbstowcs(pNewbuf,(char*)buf,l_iNewBufferSize+1);
		SystemErrorCheck();
		delete [] buf;
		buf = pNewbuf;
		buf[l_iNewBufferSize] = L'\0';
	}
	else
	if( buf[1] == 0 )
	{
		...
		buf = 0;
		SystemErrorCheck();
		UT::ErrorMsg(L"this is utf32 xml file,only support utf16 or ansii",(WCHAR*)this->Value());
		SystemErrorCheck();
		return false;	
	}
	int	l_StringLength = (int)wcslen(buf);
	buf[l_StringLength] = L'\0';
...
{
	WCHAR buf [64];
	swprintf(buf, sizeof(buf), L"%d", _value);
	SetValue (buf);
}
...
{
	WCHAR buf [256];
	swprintf( buf, sizeof(buf), L"%lf", _value);
	SetValue (buf);
}
...
const TiXmlAttribute* TiXmlAttributeSet::Find( const WCHAR* name ) const
{
	for( const TiXmlAttribute* node = sentinel.next; node != &sentinel; node = node->next )
	{
...
/*
TiXmlAttribute*	TiXmlAttributeSet::Find( const WCHAR* name )
{
	for( TiXmlAttribute* node = sentinel.next; node != &sentinel; node = node->next )
	{
		if ( wcscmp( node->name.c_str(), name ) == 0 )
