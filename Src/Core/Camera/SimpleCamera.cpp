...
namespace FATMING_CORE
{
	...
	cMatrix44 Projection::GetMatrix() const
	{
		...
		{
			Proj = cMatrix44PerspectiveRH( m_Width, m_Height, m_ZNear, m_ZFar );
		}
		else if( m_Type == Projection::Orthographic )
		{
			Proj = cMatrix44OrthographicRH( m_Width, m_Height, m_ZNear, m_ZFar );
		}
		else
			assert( 0 );
	...
	void Projection::SetFovYAspect( float FovY, float Aspect, float ZNear, float ZFar )
	{
		...
		m_Height = 2.0f * m_ZNear * tanf(FovY/2.f);
	...
	void Projection::SetFovXAspect( float FovX, float Aspect, float ZNear, float ZFar )
	{
		...
		m_Width = 2.0f * m_ZNear * tanf(FovX/2.f);
	...
	void cCamera::Render( void )
	{
		...
		SetupShaderViewProjectionMatrix(l_Mat);
	}
	...
	cFrameCamera::cFrameCamera(cFrameCamera*e_pFrameCamera)
	{
		m_Projection = e_pFrameCamera->m_Projection;
		this->SetLocalTransform(e_pFrameCamera->GetLocalTransform());
		this->SetParent(e_pFrameCamera->GetParent());
	}

	void	cFrameCamera::Render()
	{
		//glEnable(GL_DEPTH_TEST);
		FATMING_CORE::SetupShaderViewProjectionMatrix(m_Projection.GetMatrix()*this->GetWorldView());
	}
