...
namespace FATMING_CORE
{
	void	cCameraBehaveByMouseBehave::Update(float e_fElpaseTime)
	{
		...
		if(this->m_MouseBehave.GetMouseState().bLefButton)//Rotate camera
		{
			...
			m_pCamera->SetLocalTransform(cMatrix44::YAxisRotationMatrix(l_fRotateY*m_fMoveSpeed)*m_pCamera->GetLocalTransform()*cMatrix44::XAxisRotationMatrix(l_fRotateX*m_fMoveSpeed));
