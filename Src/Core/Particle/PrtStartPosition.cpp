...
namespace FATMING_CORE
{
	...
	void	cPrtStartPositionInitByFrame::SetSubFrameName(WCHAR*e_pString)
	{
		...
		swprintf(m_psSubFrameName,MAX_PATH,L"%s\0",e_pString);
	}
	...
	char*	cPrtStartPositionInitByFrame::GetDataInfo()
	{
		...
		swprintf(l_temp,MAX_PATH,L"Type:%s\nModelName:%s\nSubFrameName:%s\nOffsetPos:%.2f,%.2f,%.2f\0",AttachFrameTypeToString(m_eAttachFrameType),m_pFrame->GetName(),wcslen(this->m_psSubFrameName)?m_psSubFrameName:L"",m_vOffsetPos.x,m_vOffsetPos.y,m_vOffsetPos.z);
		WCHAR_TO_CHAR(l_temp,m_sTemp);
		return m_sTemp;
	}
	char*	cPrtStartPositionInitByFrame::GetOutputDataString()
	{
		...
		WCHAR	l_temp[MAX_PATH];
		swprintf(l_temp,MAX_PATH,L"%d,%s,%s,%.2f,%.2f,%.2f\0",(int)m_eAttachFrameType,m_pFrame?m_pFrame->GetName():L" ",wcslen(this->m_psSubFrameName)?m_psSubFrameName:L" ",m_vOffsetPos.x,m_vOffsetPos.y,m_vOffsetPos.z);
		WCHAR_TO_CHAR(l_temp,m_sTemp);
		return m_sTemp;
	}
	//input the output data string,and analyze it
	bool	cPrtStartPositionInitByFrame::SetDataByDataString(char*e_pString)
	{
		...
		switch(m_eAttachFrameType)
		{
			...
			case eAFT_FRAME_OF_FRAME:
				...
				{
					assert(0&&"current not support");
					//OutputDebugString(L"!!----I am lazy to write more flexible,support Frame-----!!\n");
