...
namespace FATMING_CORE
{
	...
	cPrtEmitter::cPrtEmitter(const WCHAR*e_pName)
	{
		m_pTexture = 0;
	...
	cPrtEmitter::cPrtEmitter(cPrtEmitter*e_pPrtEmitter,bool e_bPolicyFromClone)
	{
		...
		m_pvAllTexCoordinatePointer = 0;
		m_pParticleData = 0;
		m_pTexture = e_pPrtEmitter->GetTexture();
	...
	void	cPrtEmitter::SetTexture(cSimpleGLTexture*e_pTexture)
	{
		this->m_pTexture = e_pTexture;
		if(this->m_pTexture)
		{
			memcpy(m_pvAllTexCoordinatePointer,m_pTexture->UVToTwoTriangle(),sizeof(Vector2)*TWO_TRIANGLE_VERTICES_TO_QUAD_COUNT);
		}
		else
		{
		...
		if( e_pTexture )
		if( m_pTexture->Type() != cSimpleGLTexture::TypeID )
		{
