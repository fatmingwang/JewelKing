...
namespace FATMING_CORE
{
	...
	char*	cPrtTextureActDynamicTexture::GetOutputDataString()
	{
		...
		if( this->m_pEmitterOwner->GetTexture()->Type() == cPuzzleImage::TypeID )
		{
			...
			{
				sprintf(m_sTemp,"%s,\0",m_pImageIndexOfAnimation->GetCharName());
			}
		}
		return m_sTemp;
	}
	...
	{
		if( this->m_pImageIndexOfAnimation )
			sprintf(m_sTemp,"PIName:%s\nAnimationName:%ws\n\0",this->m_pEmitterOwner->GetTexture()->GetCharName(),m_pImageIndexOfAnimation->GetCharName());
		else
	...
	bool	cPrtTextureActDynamicTexture::SetDataByDataString(char*e_pString)
	{
		...
		cPuzzleImage*l_pPI = (cPuzzleImage*)this->m_pEmitterOwner->GetTexture();
		this->SetPuzzleImage(l_pPI);
		assert(l_pPI->GetImageIndexOfAnimationList()&&"ensure this pi has animation!!");
		this->m_pImageIndexOfAnimation = l_pPI->GetImageIndexOfAnimationList()->GetObject(UT::CharToWchar(l_pString));
