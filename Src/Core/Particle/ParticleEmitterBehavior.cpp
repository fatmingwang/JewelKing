...
{	
	...
	cParticleEmiterWithShowPosition::cParticleEmiterWithShowPosition(WCHAR*e_strName)
	{
		SetName(e_strName);
	...
	:cFatmingAnimationRule(e_pParticleEmiterWithShowPosition)
	{
		m_pPrtEmitter = 0;
		m_pCurveWithTime = 0;
	...
	void	cParticleEmiterWithShowPosition::PathUpdate(float e_fElpaseTime)
	{
		...
		{
			m_pPrtEmitter->SetPos(m_vEmitterPos+m_vPos);
		...
		if( ePT_FOLLOW_PATH == this->m_ePathType )
		{
			...
			m_pPrtEmitter->SetPos(this->m_pCurveWithTime->GetCurrentPosition()+m_vPos);
			m_pPrtEmitter->Update(e_fElpaseTime);
		}
		else
		if( ePT_ALL_PATH_POINT == this->m_ePathType )
		{
			if(m_pPrtEmitter->ShotUpdate(e_fElpaseTime))
			{
				...
				{
					m_pPrtEmitter->SetPos(m_pCurveWithTime->GetOriginalPointList()[i]+m_vPos);
				...
				m_pPrtEmitter->SetPos(m_pCurveWithTime->GetOriginalPointList()[0]+m_vPos);
			}
	...
	void	cParticleEmiterWithShowPosition::RearrangeTime(float e_fPercenttage)
	{
	...
	void	cParticleEmiterWithShowPosition::RenderByGlobalTime(float e_fTime,bool e_bRotateWithAngle)
	{
		this->Init();
		this->Update(e_fTime);
		this->Render();
	...
	{
		m_bAnimationDone = e_bAnimationDone;
	...
	void	cParticleEmiterWithShowPosition::RefreshAngleAndPosData()
	{
		if(this->m_pCurveWithTime)
		{
			...
			m_pCurveWithTime->AddOffsetToAllPoint(m_vEmitterPos+m_vPos);
		}
		if( m_pPrtEmitter )
