...
//please use Sphere instead circle
struct	sCircle
{
	sCircle(){}
	sCircle(Vector3 e_vPos,float e_fRadius){ fRadius = e_fRadius; vPos = e_vPos; }
	//sCircle(){}
	float				fRadius;// Radius of the sphere.
	Vector3				vPos;// Center of the sphere.
};
...
namespace UT
{
	...
	//fuck it does't work
	int 					sphere_line_intersection(Vector3	e_vP1,Vector3	e_vP2,sCircle e_sCircle,float*e_pfRssult);
	bool					CircleCollideCircleWithNewPosition(sCircle *e_pCircleTarget,sCircle *e_pCircleSource,float e_fStillCollidedOffset = 0.f );	
	...
	inline int				CircleCollideRect( RECT e_rc,sCircle e_Circle){	POINT	l_Point = {(long)e_Circle.vPos.x,(long)e_Circle.vPos.y};return CircleCollideRect(e_rc,l_Point,e_Circle.fRadius);}
