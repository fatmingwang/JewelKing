...
public:
	...
	{
		//assert(strlen(e_str)<260&&"sring is too log to parse Vector2 ");
		char	l_str[260];
		sprintf(l_str,"%s\0",e_str);
		//char*	l_str = (char*)e_str ;
		char*l_strDest = strtok(l_str,",");
		u = (float)atof(l_strDest);
		l_strDest = strtok(0,",");
		v = (float)atof(l_strDest);
	}
