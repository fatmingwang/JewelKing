...
namespace UT
{
	...
	//so work these out and apply them to the coordinates of the circle.
	//
	...
	int				CircleCollideRect( RECT e_rc,POINT e_CenterPoint,float e_fRadius)
	{
		//1
	...
	bool				CircleCollideCircle(sCircle e_Circle1,sCircle e_Circle2)
	{
		float	l_fDistance = (e_Circle1.vPos-e_Circle2.vPos).Length();
		float	l_fRadius = e_Circle1.fRadius+e_Circle2.fRadius;
		...
		float*e_pfRssult)
	{
		 ...
		 float x3 = e_sCircle.vPos.x;float y3 = e_sCircle.vPos.y;float z3 = e_sCircle.vPos.z;float r = e_sCircle.fRadius;
		 float a, b, c, mu, i ;
	...
	//the resource circle will adjust to the new position for suitable place where seeing well
	//e_fStillCollidedOffset is the value let 2 object has a small collide zone but for vision will be non-notificationed
	//
	bool				CircleCollideCircleWithNewPosition(sCircle *e_pCircleTarget,sCircle *e_pCircleSource,float e_fStillCollidedOffset)
	{
		Vector3	l_vRelativePos = e_pCircleTarget->vPos-e_pCircleSource->vPos;
		float	l_fLength = l_vRelativePos.Length();
		float	l_f2Radius = e_pCircleSource->fRadius+e_pCircleTarget->fRadius-e_fStillCollidedOffset ;
		if( l_fLength <= l_f2Radius )
		{
			e_pCircleSource->vPos = e_pCircleTarget->vPos-(l_vRelativePos.Normalize()*(l_f2Radius));
			return true;
		}
		return false;
	}
