...
namespace FATMING_CORE
{
	...
	void	cCurve::RenderPointIndex()
	{
		if( !cGameApp::m_spGlyphFontRender )
			return;
		int	l_iNumPoint = (int)m_OriginalPointList.size()-1;
		if( l_iNumPoint == -1 )
			return;
	...
	cCurveWithTime::cCurveWithTime(cCurveWithTime*e_pCurveWithTime):cCurve(e_pCurveWithTime)
	{
	...
	float	cCurveWithTime::GetLastTime()
	{
		if( m_OriginalTimeList.size()>0 )
			return m_OriginalTimeList[m_OriginalTimeList.size()-1];
		return 0.f;
	}
	...
	void	cCurveWithTime::MovedDistanceCalculate(float e_fElpaseTime)
	{
		...
		{
			assert(0&&"play back is not allowed!!");
			return;
		}
	#endif
	...
	void	cCurveWithTime::Update(float e_fElpaseTime)
	{
	...
	{
		//int l_iLastIndex = (int)m_OriginalPointList.size()-1;
		////only 1 or 0 data is not necessery to invert
		//if( l_iLastIndex <1 )
		//	return;
		//UINT l_iSize = this->m_OriginalTimeList.size();
		//std::vector<Vector3> l_OriginalPointList = m_OriginalPointList;
		//for( UINT i=0;i<l_iSize;++i )
		//{
		//	l_OriginalPointList[l_iLastIndex-i] = m_OriginalPointList[i];
		//}
		//m_OriginalPointList = l_OriginalPointList;
		UT::InvertVectorDateOrder(&m_OriginalPointList);
		cCurve::DoLOD();
	}
	...
	{
		float	l_fRestTime = GetLastTime()-this->m_fCurrentTime;
		return l_fRestTime;
	}
