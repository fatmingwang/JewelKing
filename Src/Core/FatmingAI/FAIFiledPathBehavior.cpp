...
namespace FATMING_AI
{
	...
	void    cFAIFiledPathBehavior::InternalInit()
	{
		...
		sMinMaxData<float>	l_DirectionOffset;
		l_DirectionOffset.Max = 0.1f;
		l_DirectionOffset.Max = -0.1f;
		float	l_fSpeed = this->m_pSrcCharacter->GetSpeed();
		for( int i=0;i<l_iNumPoints;++i )
		{
			l_fTargetTime = l_fTime*(i+1);
			Vector3	l_vNewPos(m_fXDistance.Rand()*l_vDirection.x*l_fSpeed,m_fYDistance.Rand()*l_vDirection.y*l_fSpeed,0.f);
	...
	void    cFAIFiledPathBehavior::InternalUpdate(float e_fElpaseTime)
	{
		...
		if(m_CurveWithTime.IsCurveMoveDone())
		{
			...
			if( l_pBound )
			{
				...
				{
					l_fBound = l_pBound->GetSphere().Radius;
				}
			}
			...
			float	l_fSpeed = this->m_pSrcCharacter->GetRunAwaySpeed();
