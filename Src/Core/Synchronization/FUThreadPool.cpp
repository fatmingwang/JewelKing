...
:	threads()
,	workQueue(), workQueueCS(NULL), killAllFlag(true), idleThreadCount(0)
{
...
bool cFUThreadPool::WaitForIdle(uint32 maxWaitTime)
{
	while (maxWaitTime >= 0)
	{
		...
		else if (maxWaitTime <= 10) { cFUThread::SleepCurrentThread(maxWaitTime); maxWaitTime = 0; }
		else { cFUThread::SleepCurrentThread(10); maxWaitTime -= 10; }
	}
	return false;
}
...
ImplementThreadRoutine(cFUThreadPool::WorkerThread, threadPool)
{
	...
	while (!isDone)
	{
		...
		cFUThreadPool::WorkItem* workItem = NULL;
