...
namespace FATMING_CORE
{
	...
	public:
		cTabPage();
		cTabPage(Vector4 e_vWorkingZone,cImageButton*e_pCancelButton = 0,cBaseImage*e_pIcon = 0,Vector3 *e_pvIconPos = 0,bool e_bForceSetCancelButtonPos = false);
		cTabPage(cTabPage*e_pTabPage);
		CLONE_MYSELF(cTabPage);
		~cTabPage();
		void			SetIconColor(Vector4 e_vColor){if( m_pIcon )m_pIcon->SetColor(e_vColor);}
		void			RenderIcon();
		bool			IsCollideIcon(int e_iPosX,int e_iPosY);
		virtual	void	SetTabPageIcon(cBaseImage*e_pIcon);
		cBaseImage*		GetTabPageIcon(){return m_pIcon;}
		...
		virtual	void	AddData(cClickMouseBehavior*e_pClickMouseBehavior);
		//
		virtual	void	AddData(cPuzzleImage*e_pPuzzleImage,bool e_bClearOldData = false);
		virtual	void	AddData(std::vector<cBaseImage*>*e_pImageList,bool e_bClearOldData = false);
	};
	//================
	//this class is a cTabPage list controller
	//but it's allow add the object is not TabPage
	//================
	class	cTabPageControl:public cMouse_TouchWithCancelButton//public cClickMouseBehaviorList<cTabPage>
	{
		cCueToStartCurveWithTime*m_pBG;
	public:
		//if e_pBG is not null,after this is delete e_pBG will delete too.
		cTabPageControl(Vector4 e_vWorkingZone,cImageButton*e_pCancelButton = 0,cCueToStartCurveWithTime*e_pCueToStartCurveWithTime = 0,bool e_bForceSetCancelButtonPos = false);
		~cTabPageControl();
		//
