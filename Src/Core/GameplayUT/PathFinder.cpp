...
namespace FATMING_CORE
{
	...
	void	cLinkedNodes::RemoveObject(POINT e_Coordinate)
	{
		...
		for( int i=0;i<l_iCount;++i )
		{
			if(this->m_ObjectList[i]->IsSameCoordinate(e_Coordinate))
			{
				...
				cObjectListByName<cLinkedNodes>::RemoveObject(l_iIndex);
			}
		}	
	}
	...
	bool	cLinkedNodes::FindTargetNode(cLinkedNodes*e_pTarget,cObjectListByName<cLinkedNodes>*ParentPath,cObjectListByName<cObjectListByName<cLinkedNodes> >*e_pResultPathes,bool e_bFindShoest)
	{
		ParentPath->AddObjectNeglectExist(this);
		//only need shorest path so compare it
		if( e_bFindShoest && e_pResultPathes->Count() )
		{
			int	l_iCount = e_pResultPathes->GetObject(0)->Count();
			if( ParentPath->Count() > l_iCount )
				return false;
		}
		...
		{//u could compare count if the count is bigger than the path we found,ignore it,so we could find the smallest one
			...
			if( e_bFindShoest  )
			{
				...
				{
					int	l_iCount = e_pResultPathes->GetObject(0)->Count();
					int	l_iNewPathCount = ParentPath->Count();
					//only need shorest
			...
			cObjectListByName<cLinkedNodes>*l_pResult = new cObjectListByName<cLinkedNodes>(true);
			l_pResult->CopyListPointer(ParentPath);
			e_pResultPathes->AddObjectNeglectExist(l_pResult);
			return true;
		}
		if(  ParentPath->Count() >= m_siMaxAllowCount )
		{
			return false;		
		}
		...
		for( int i=0;i<l_iCount;++i )
		{
			...
			if( ParentPath->GetObjectIndexByPointer(l_pChild) != -1 )
				continue;
			cObjectListByName<cLinkedNodes>l_ObjectListByName(true);
			l_ObjectListByName.CopyListPointer(ParentPath);
	...
	bool	cLinkedNodes::FindTargetNode(cLinkedNodes*e_pTarget,cObjectListByName<cObjectListByName<cLinkedNodes> >*e_pResultPathes,bool e_bFindShoest)
	{
		bool	l_bFind = false;
		//linked object
		int	l_iCount = this->Count();
	...
	bool	cLinkedMapManager::FindTargetNode(POINT e_Point1,POINT e_Point2,cObjectListByName<cObjectListByName<cLinkedNodes> >*e_pResultPathes,bool e_bFindShoest)
	{
		if( !e_pResultPathes )
			return false;
