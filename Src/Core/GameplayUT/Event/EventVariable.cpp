...
namespace FATMING_CORE
{
	...
	cEventVariable::cEventVariable(WCHAR*e_strName,eDataType e_eDataType,const WCHAR*e_strValue)
	{
		m_pData = 0;
		...
		switch( m_eDataType )
		{
			...
			case eDT_UINT64:
				*(uint64*)m_pData = FATMING_CORE::GetUint64(e_strValue);
				break;
			case eDT_INT64:
				*(uint64*)m_pData = FATMING_CORE::GetInt64(e_strValue);
				break;
			default:
	...
	{
		return cObjectListByName<cEventVariable>::RemoveObject(e_iIndex);
	}
	bool	cEventVariableList::AddObjectNeglectExist(cEventVariable* e_pcEventVariable)
	{
		return cObjectListByName<cEventVariable>::AddObjectNeglectExist(e_pcEventVariable);
	}
	...
	void			cEventVariableManager::Export(char*e_strFileName)
	{
