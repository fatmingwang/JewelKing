...
namespace FATMING_CORE
{
	...
	void	cEventExternalFunctionCallActiver::InternalUpdate(float e_fElpasedTime)
	{
		...
		if( l_pExternalFunction )
		{
			...
			cEventInstance*l_pEventInstance = dynamic_cast<cEventInstance*>(l_pParent);
			l_str = l_pEventInstance->Type();
			l_pExternalFunction->m_pDoButtonGoal_Callback(l_pEventInstance);
		}
