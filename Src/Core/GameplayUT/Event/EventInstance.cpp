...
namespace FATMING_CORE
{
	...
	bool	cEventInstance::UpdateEventList(cObjectListByName<cEventBase>*e_EventList,float e_fElpasedTime)
	{
	...
	void	cEventInstance::RenderEventList(cObjectListByName<cEventBase>*e_EventList)
	{
		int	l_iNum = e_EventList->Count();
		cEventBase**l_ppEvent = 0;
		cEventBase*l_pEvent = 0;
	...
	{
		LIST_DO_FOR_ALL_CHILDREN((&m_TriggerEventList),FetchData(this->GetName(),this));
		LIST_DO_FOR_ALL_CHILDREN((&m_ActiveEventList),FetchData(this->GetName(),this));
	...
	void	cEventInstance::CollectResourceList(cObjectListByName<NamedTypedObject>*e_pObjectListByName)
	{
