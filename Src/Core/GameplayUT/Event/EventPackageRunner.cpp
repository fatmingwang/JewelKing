...
namespace   FATMING_CORE
{
    ...
    void    cEventPackageRunner::Update(float e_fElpaseTime)
    {
        ...
        if(m_pCurrentEventPackage)
		{
			...
			if( l_pNameAndData )
			{
				if( wcscmp(m_strNewPageName.c_str(),l_pNameAndData->GetWString()->c_str()) )
				{
	...
	void	cEventPackageRunner::RemoveUnUsingEventInstanceAtEventManager(cEventPackage*e_pOldEventPackage,cEventPackage*e_pNewEventPackage)
	{
		...
		cObjectListByName<cEventInstance>	l_EventInstanceList;
		int	l_iNewEventPackageCount = e_pNewEventPackage->Count();
		int	l_iOldEventPackageCount = e_pOldEventPackage->Count();
		for( int i=0;i<l_iOldEventPackageCount;++i )
		{
		...
		for( int i=0;i<l_iNewEventPackageCount;++i )
		{
			...
			cClickMouseBehaviorList<cEventInstance>*l_pEventInstanceList = l_pNewEventObjectInstance->GetEventInstanceList();
			int	l_iEventInstanceCount = l_pEventInstanceList->Count();
			for( int j=0;j<l_iEventInstanceCount;++j )
			{
				l_EventInstanceList.RemoveObjectWithoutDelete(l_pEventInstanceList->GetObject(j)->GetName());
			}
		}
	...
	{
		cObjectListByName<NamedTypedObject>	l_OldResourceList(true);
		cObjectListByName<NamedTypedObject>	l_NewResourceList(true);
		cObjectListByName<NamedTypedObject>	l_ReleaseResourceList(true);
		int	l_iNewEventPackageCount = e_pNewEventPackage->Count();
		int	l_iOldEventPackageCount = e_pOldEventPackage->Count();
		for( int i=0;i<l_iOldEventPackageCount;++i )
		{
