...
namespace FATMING_CORE
{
	...
	//const wchar_t*         cEventImageButton::TypeID( L"cEventImageButton" );
	//const wchar_t*         cEventImageButton::TypeID( L"cEventImageButton" );
	...
	cEventButton::cEventButton(TiXmlElement*e_pTiXmlElement)
	{
		...
		PARSE_ELEMENT_START(e_pTiXmlElement)
			...
			COMPARE_NAME_WITH_DEFINE(cPuzzleImage::TypeID)
			{
				...
				if( !l_pPI )
				{
					std::string	l_strFileName = g_pCurrentEventManager->GetFullpathFileName(UT::CharToWchar( cPuzzleImage::GetFileName((WCHAR*)l_strValue) ));
			...
			COMPARE_NAME_WITH_DEFINE(EVENT_REPEAT)
			{
				m_bRepeatEnable = VALUE_TO_BOLLEAN;
			}
			else
	...
	const WCHAR*			ValueToString(eButtonSelectionType e_eButtonSelectionType)
	{
		switch(e_eButtonSelectionType)
		{
			case eBST_ONE:
				swprintf(g_sstrForStringToStructureW,MAX_PATH,L"One");
				break;
			case eBST_ALL_CLICK:
				swprintf(g_sstrForStringToStructureW,MAX_PATH,L"AllClick");
				break;
			case eBST_REPEAT:
				swprintf(g_sstrForStringToStructureW,MAX_PATH,EVENT_REPEAT);
				break;
			case eBST_ALL_PRESSED:
				swprintf(g_sstrForStringToStructureW,MAX_PATH,L"AllPressed");
				break;
			default:
				swprintf(g_sstrForStringToStructureW,MAX_PATH,L"unknow");
				break;
		}
	...
	cEventMultiButton::cEventMultiButton(TiXmlElement*e_pTiXmlElement)
	{
		...
		PARSE_ELEMENT_START(e_pTiXmlElement)
			...
			{
				m_bStayAtLastSelect = VALUE_TO_BOLLEAN;
			}
			else
	...
	void	cEventMultiButton::CollectResourceList(cObjectListByName<NamedTypedObject>*e_pObjectListByName)
	{
		int	l_iCount = this->Count();
		for( int i=0;i<l_iCount;++i )
		{
