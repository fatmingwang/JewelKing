...
namespace FATMING_CORE
{
    ...
    cEventManager::cEventManager()
    {
		...
		m_pEventFont = 0;
		m_pExternalEventPackage = new cObjectListByName<cEventPackage>;
		m_pEventInstanceList = new cObjectListByName<cEventInstance>;
		m_pEventObjectStatusList = new cObjectListByName<cEventObjectStatus>;
		m_pEventVariableManager = new cEventVariableManager();
		m_pExternalFunctionList = new cObjectListByName<cExternalFunction>;
	...
	char*	cEventManager::GetFullpathFileName(const WCHAR*e_strName)
	{
		sprintf(g_sstrForStringToStructure,"%s%s",m_pstrWorkingDirectory->c_str(),UT::WcharToChar(e_strName));
...
//	</EventPackage>
	...
	cEventObjectInstance*	cEventManager::GetEventObjectInstance(const WCHAR*e_strEventObjectInstanceName,const WCHAR*e_strEventInstanceName,const WCHAR*e_strEventObjectStatusName,bool e_bAllStatusWorking,const WCHAR*e_strEventVariableName)
	{
		...
		if( wcslen(e_strEventObjectInstanceName) )
			l_pEventObjectInstance->SetName(e_strEventObjectInstanceName);
		if( wcslen(e_strEventInstanceName) )
		{
			l_pEventInstance = GetEventInstance(e_strEventInstanceName);
			if( l_pEventInstance )
				l_pEventObjectInstance->AddEventInstance(l_pEventInstance);
		}
		if( wcslen(e_strEventObjectStatusName) )
		{
			l_pEventObjectStatus = GetEventObjectStatus(e_strEventObjectStatusName);
			if( l_pEventObjectStatus )
			{
				l_pEventObjectInstance->SetTargetEventObjectStatus(l_pEventObjectStatus);
			}
			else
				UT::ErrorMsg(L"there is no cEventObjectStatus",e_strEventObjectStatusName);
		}
		if( wcslen(e_strEventVariableName) && l_pEventObjectStatus )
		{
			cEventVariable*l_pEventVariable = this->m_pEventVariableManager->GetObject(e_strEventVariableName);
			if( l_pEventVariable )
			{
				l_pEventObjectInstance->SetStatusEventVariable(l_pEventVariable);
			}
			else
			{
				UT::ErrorMsg(L"GetEventObjectInstance there is no cEventVariable",e_strEventVariableName);
			}
		}
	...
	{
		cEventObjectInstance*l_pEventObjectInstance = GetEventObjectInstance(e_strEventObjectInstanceName,e_strEventInstanceName,e_strEventObjectStatusName,e_bAllStatusWorking,e_strEventVariableName);
		if( !l_pEventObjectInstance )
			return 0;
		cEventPackage*l_pEventPackage = this->m_pEventPackageRunner->GetCurrentEventPackage();
		if( !l_pEventPackage )
		{
			delete l_pEventObjectInstance;
			return 0;
		}
		if( l_pEventPackage->GetObject(l_pEventObjectInstance->GetName()) )
		{
			delete l_pEventObjectInstance;
			return 0;
		}
