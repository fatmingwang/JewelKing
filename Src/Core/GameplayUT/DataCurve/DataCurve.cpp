...
namespace FATMING_CORE
{
	...
	void	cDataAndCurve::Render(float e_fMaxMinScale,Vector4 e_vColor,Vector4 e_vRenderRect,bool e_bDrawRect)
	{
		...
		if( e_bDrawRect )
		{
			...
			l_str = UT::ComposeMsgByFormat(L"Start:%.2f",this->m_dbMinValue);
	...
	void		cDataAndCurveListWithXMlSechma::ProcessRootData(TiXmlElement*e_pTiXmlElement)
	{
		...
		for( size_t i=0;i<l_uiSize;++i )
		{
			...
			for( int j=0;j<l_iNumData;++j )
			{
				...
				//OutputDebugString(ValueToStringW(l_dbFinalValue));
				//OutputDebugString(L",");
	...
	void		cDataAndCurveListWithXMlSechma::Render(WCHAR*e_strName,Vector4 e_vRenderRect)
	{
