#ifndef _IMAGE_WITH_SLIDER_H_
#define _IMAGE_WITH_SLIDER_H_
#include "Scroller.h"
namespace FATMING_CORE
{
	...
	//
	//=====================
	class	cImageSliderTabPages:public cTabPageControl
	{
		//slider bar bk
		cMultiPathDynamicImage*m_pSliderBarBK;
	public:
		cImageSliderTabPages(Vector4 e_vWorkingZone,cMultiPathDynamicImage*e_pSliderBarBK,cImageButton*e_pCancelButton = 0);
		~cImageSliderTabPages();
        //virtual void    MouseUp(int e_iPosX,int e_iPosY);
		virtual	void    Update(float e_fElpaseTime);
		virtual	void    Render();

		void	SetFocusItem(int e_iTabPageIndex,cPuzzleImageUnit*e_pPIUnit);
		//void	SetWorkingTabPage(int e_iTabPageIndex);
		//NamedTypedObject*	GetSelectedObject();
	};
	...
	//									"NL1/InHouse/Props/no.pi"};
	//	WCHAR*l_strAllPIName[5] = {		L"Characters",
	//									L"Floor",
	//									L"Props",
	//									L"no",
	//									L"no"};
	...
	//	m_pBuildItemTabePage = new cImageSliderTabPages(l_vScrollerRect,l_pSliderBarBK,l_pImageButton);
	...
	//			}
	//			if( l_pPIUnit )
	//				l_vScrollerRect.y = l_vFirstIconImagePos.y+(*l_pPIUnit->GetImageShowHeight());
	//			else
