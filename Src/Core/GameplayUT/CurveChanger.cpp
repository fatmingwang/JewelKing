...
namespace FATMING_CORE
{
	...
	cCurveChanger::cCurveChanger(cObjectListByName<cCurveWithTime> *e_pCurveWithTimeVector)
	{
		m_bChangeCurve = false;
		m_iCurrentCurveIndex = 0;
		m_pCurrentCurveWithTime = 0;
		CLONE_VECTOR_TO_NEW_VECTOR(cCurveWithTime,e_pCurveWithTimeVector,((cObjectListByName<cCurveWithTime>*)&m_CurveWithTimeVector));
	...
	cCurveChanger::cCurveChanger(cCurveChanger*e_pCurveChanger)
	{
		...
		CLONE_VECTOR_TO_NEW_VECTOR(cCurveWithTime,((cObjectListByName<cCurveWithTime>*)&e_pCurveChanger->m_CurveWithTimeVector),((cObjectListByName<cCurveWithTime>*)&m_CurveWithTimeVector));
		//or instead set data is loop from path file
		for( int i=0;i<m_CurveWithTimeVector.Count();++i )
		{
			m_CurveWithTimeVector[i]->SetCurveLoop(true);
