	...
	public: System::Void RefreshPI()
			 {
				...
				if( g_pPaticleManager )
				{
					if( g_pPaticleManager->GetImageParser() )
					{
						...
						{
							cSimpleGLTexture*l_pSimpleGLTexture = dynamic_cast<cSimpleGLTexture*>(g_pPaticleManager->GetImageParser()->GetObject(i));
							if( l_pSimpleGLTexture->Type() == cPuzzleImage::TypeID )
							{
								cPuzzleImage*l_pPI = dynamic_cast<cPuzzleImage*>(l_pSimpleGLTexture);
		...
		private: System::Void PI_listBox_SelectedIndexChanged(System::Object^  sender, System::EventArgs^  e)
		 {
			 ...
			 {
				 if( g_pPrtTextureActDynamicTexture->GetEmitterOwner()->GetTexture() )
				 {
					 ...
					 cObjectListByName<cImageIndexOfAnimation>*l_pImageIndexOfAnimationList = l_pPI->GetImageIndexOfAnimationList();
