...
namespace FMBookEditor
{
	...
	System::Void EventPackages_Editor::Revert(System::Object^  sender, System::EventArgs^  e)
	{
		if(m_pEventPackages->m_Packages_listBox->SelectedIndex != -1)
		{
			...
			cEventPackage*l_pEventPackage = g_pFMBookApp->m_spEventManager->GetEventPackage(l_strPackageName);
			cEventObjectInstance*l_pEventObjectInstance = 0;
			if(l_pEventPackage)
			{
				g_pFMBookApp->m_spEventPackageRunner->SetCurrentEventPackage(l_pEventPackage);
	...
	void	 EventPackages_Editor::Refresh()
	{
		...
		TiXmlElement*l_pTiXmlElement = g_pFMBookApp->m_spEventManager->GetRootElement();//->ChildElementCount()
		if( l_pTiXmlElement )
		{
