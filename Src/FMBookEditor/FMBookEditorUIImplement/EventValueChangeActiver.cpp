...
namespace FMBookEditor
{
	...
	EventValueChangeActiver_Editor::EventValueChangeActiver_Editor(FMBookEditorUI::Event::Activer::EventValueChangeActiver^e_pEventValueChangeActiver,cEventValueChangeActiver*e_pEventValueChangeActiver2)
	{
		...
		cEventManager*l_pEventManager = cFMBookApp::m_spEventManager;
		...
		switch( l_iSelectedIndex )
		{
			...
			case 2:
			{
				...
				else
				{
					...
					float	l_fMax = m_pEventValueChangeActiverUI->m_RestrictValue1->m_Max->Value;
					float	l_fMin = m_pEventValueChangeActiverUI->m_RestrictValue1->m_Min->Value;
					float	l_fIncrease = m_pEventValueChangeActiverUI->m_RestrictValue1->m_IncreaseValue->Value;
