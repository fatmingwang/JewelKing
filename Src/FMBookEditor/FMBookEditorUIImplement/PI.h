...
namespace FMBookEditor
{
	...
	public:
		PI(CoreWPF::FMC::PI^e_pUserControl1)
		{
			...
			e_pUserControl1->f_AddPI += gcnew EventHandler(this,&PI::AddPI_button_Click);
			e_pUserControl1->f_DelPI += gcnew EventHandler(this,&PI::DeletePI_button_Click);
			
			PIUnitList_listBox = e_pUserControl1->m_PIUnit_listBox;
			PIList_listBox = e_pUserControl1->m_PI_listBox;
			m_pImageParser = 0;
			m_pPuzzleImageList = new cObjectListByName<cPuzzleImage>(true);
    ...
    public:void    RefreshTextureID(){ if(m_pCurrentPIUnit)m_pCurrentPIUnit->m_suiLastUsingImageIndex = -1; }
    private:cObjectListByName<cPuzzleImage>*m_pPuzzleImageList;
