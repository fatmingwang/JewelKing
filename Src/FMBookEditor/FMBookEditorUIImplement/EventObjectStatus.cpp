...
namespace FMBookEditor 
{
	...
	System::Void EventObjectStatus::UICombine(cEventObject*e_pEventObject)
	{
		...
		{
			this->m_pEventObjectStatus->m_ImagePosXYZNumeric->SetValue((float*)e_pEventObject->GetPos());
		...
		{
			this->m_pEventObjectStatus->m_TextPosXYZNumeric->SetValue((float*)e_pEventObject->GetPos());
		}
		else
		if( e_pEventObject->Type() == cEventMPDI::TypeID)
		{
			...
			m_pEventObjectStatus->m_MPDIPosXYZNumeric->SetValue((float*)l_pEventMPDI->GetPos());
		...
		if( g_pEditorEventObject->Type() == cEventVariableDataRender::TypeID)
		{
			...
			cEventVariable*l_pEventVariable = g_pFMBookApp->m_spEventManager->GetEventVariable(DNCT::GcStringToWchar(m_pEventObjectStatus->m_VariableRenderEventVariableName->Text));
			l_pEventVariableDataRender->SetEventVariable(l_pEventVariable);
			m_pEventObjectStatus->m_VariableRenderPosNumeric->SetValue((float*)l_pEventVariableDataRender->GetPos());
		}
		else
	...
	System::Void EventObjectStatus::StatusApplyTest_button_Click(System::Object^  sender, System::EventArgs ^  e)
	{
		...
		switch(m_eEventObjectType)
		{
			...
			case eEOT_IMAGE:
				...
				{
					l_vSize.x = (float)*m_pFatmingUtilitiesData->m_pPI->m_pCurrentPIUnit->GetImageShowWidth();
					l_vSize.y = (float)*m_pFatmingUtilitiesData->m_pPI->m_pCurrentPIUnit->GetImageShowHeight();
				...
				if( g_bSelecteImageIsPI == false )
				{
					...
					l_vSize.x = (float)*l_pImage->GetImageShowWidth();
					l_vSize.y = (float)*l_pImage->GetImageShowHeight();
			...
			case eEOT_MPDI:
				if(m_pFatmingUtilitiesData->m_pMPDI->m_pCurrentMPDI)
				{
					l_vSize.x = 1.f;
					l_vSize.y = 1.f;
					m_pFatmingUtilitiesData->m_pMPDI->m_pCurrentMPDI->SetLoop(true);
