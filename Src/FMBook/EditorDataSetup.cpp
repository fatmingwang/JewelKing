...
{
	//
	cEventPackage*l_pEventPackage = cFMBookApp::m_spEventPackageRunner->GetCurrentEventPackage();
	if( l_pEventPackage )
	{
	...
	cFMBookApp::m_spEventPackageRunner->SetCurrentEventPackage(sFMBookEditorData::m_spEventPackage);
}
...
sFMBookEditorData::~sFMBookEditorData()
{
	//all data is store at cFMBookApp::m_spEventPackageRunner so do not kill it.
}
...
{
	//cNodeISAX*g_pFMBookApp->m_spEventManager->m_pEventObjectStatusXml;
	//cNodeISAX*g_pFMBookApp->m_spEventManager->m_pEventInstanceXml;
	//cNodeISAX*l_pNodeISAX = g_pFMBookApp->m_spEventManager->m_pVariableManagerXml;
	TiXmlElement*l_pRootTiXmlElement = g_pFMBookApp->m_spEventManager->GetRootElement();
	if( !l_pRootTiXmlElement )
	{
		...
		g_pFMBookApp->m_spEventManager->SetDoc(l_pDoc);
		g_pFMBookApp->m_spEventManager->SetRootElement(l_pRootTiXmlElement);
	...
	TiXmlElement*l_pOriginalTiXmlElement = g_pFMBookApp->m_spEventManager->GetChildOfRootXmlElementByName(l_strNewPackageName);
...
void	sFMBookEditorData::DelEventPackage(WCHAR*e_strName)
{
	...
	TiXmlElement*l_pTargetTiXmlElement = g_pFMBookApp->m_spEventManager->GetChildOfRootXmlElementByName(e_strName);
...
void	sFMBookEditorData::SaveFile(char*e_strName)
{
	...
	//for eventinstance
	{
		...
		int	l_iCount = g_pFMBookApp->m_spEventManager->m_pEventInstanceList->Count();
		for( int i=0;i<l_iCount;++i )
		{
			TiXmlElement*l_pEventInstance = g_pFMBookApp->m_spEventManager->m_pEventInstanceList->GetObject(i)->ToTiXmlElement();
			if( !l_pEventInstance )
			{
				UT::ErrorMsg(g_pFMBookApp->m_spEventManager->m_pEventInstanceList->GetObject(i)->Type(),L"to tixml element not implement");
			}
			else
			{
		...
		ReplaceDocumentwithNewElement(g_pFMBookApp->m_spEventManager->m_pEventInstanceXml,l_pEventInstanceTiXmlElement);
	}
	//for eventObjectinstance
	{
		TiXmlElement*l_pEventObjectsTiXmlElement = new TiXmlElement(L"EventObjectStatusList");
		int	l_iCount = g_pFMBookApp->m_spEventManager->m_pEventObjectStatusList->Count();
		for( int i=0;i<l_iCount;++i )
		{
			TiXmlElement*l_pEventStatus = g_pFMBookApp->m_spEventManager->m_pEventObjectStatusList->GetObject(i)->ToTiXmlElement();
			if( !l_pEventStatus )
			{
				UT::ErrorMsg(g_pFMBookApp->m_spEventManager->m_pEventObjectStatusList->GetObject(i)->Type(),L"to tixml element not implement");
			}
			else
			{
		...
		ReplaceDocumentwithNewElement(g_pFMBookApp->m_spEventManager->m_pEventObjectStatusXml,l_pEventObjectsTiXmlElement);
	}
	//event variable
	{
		//std::string	l_strVariableName = l_strDirectory.c_str();
		//l_strVariableName += "/EventVariableData.xml";
		//g_pFMBookApp->m_spEventManager->m_pEventVariableManager->Export((char*)l_strVariableName.c_str());
	}
	g_pFMBookApp->m_spEventManager->Export(e_strName);
}
...
void	ConvertXMLDataToListAndCleanXmlData()
{
	...
	if( l_pEventManager->m_pEventObjectStatusXml )
	{
		...
		if( l_pEventStatusRoot )
		{
			...
			{
				cEventObjectStatus*l_pEventObjectStatus = g_pFMBookApp->m_spEventManager->GetEventObjectStatus(l_pEventStatusRoot);
	...
	{
		TiXmlElement*l_pEventInstanceRoot = g_pFMBookApp->m_spEventManager->m_pEventInstanceXml->GetRootElement();
		if( l_pEventInstanceRoot )
		{
			l_pEventInstanceRoot = l_pEventInstanceRoot->FirstChildElement();
			while( l_pEventInstanceRoot )
			{				
				cEventInstance*l_pEventInstance = g_pFMBookApp->m_spEventManager->GetEventInstance(l_pEventInstanceRoot);
				if( l_pEventInstance )
				{
...
{
	 g_pFMBookApp->Destroy();
	 if(g_pFMBookApp->m_spEventManager->ParseDataIntoXMLNode(e_strName))
	 {
		sFMBookEditorData::DestroyResource();
		cFMBookApp::m_spEventPackageRunner->SetCurrentEventPackage(0);
		g_pFMBookApp->m_spEventManager->m_pEventPackageRunner->LoadPackage();
